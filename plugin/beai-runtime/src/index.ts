...
import {
  ...
  buildContinuityJudgmentProfile,
  buildTurnPlan,
...
type ContextMeshEnforcementBlock = {
  kind: "context_mesh_must_read";
  currentInput: string;
  mustReadBeforeAnswer: boolean;
  hits: ContextMeshLoadedHit[];
...
function stripMetaEnvelope(text: string): string {
  ...
  if (metadataFenceStripped !== trimmed) return stripMetaEnvelope(metadataFenceStripped);
...
function stripLeadingMetadataFences(text: string): string {
  ...
  for (let guard = 0; guard < 5; guard += 1) {
    ...
    if (!looksLikeMetadataOnly(fencedBody)) break;
...
function extractLikelyUserTextFromJsonEnvelope(text: string): string | undefined {
...
function findUserTextField(value: unknown, depth = 0): string | undefined {
  ...
  for (const key of ["currentInput", "current_input", "userText", "user_text", "text", "message", "content"]) {
    ...
    if (typeof field === "string" && field.trim() && !looksLikeMetadataOnly(field)) return field;
  }
...
function looksLikeMetadataOnly(text: string): boolean {
  ...
  if (/^(telegram|sender|conversation|session|messageId|chatId|updateId|delivery)\s*[:=]/i.test(normalized)) return true;
  if (
    /^[{[]/.test(normalized) &&
...
function isExplicitContinuityResumeRequest(input: string | undefined): boolean {
  return /((이전\s*(?:흐름|대화).{0,24}(?:이어|계속|복구|재개))|이어\s*받|이어받|(?:새|다음)\s*세션.{0,24}(?:이어|인계|handoff|context|컨텍스트)|세션\s*연속성|handoff|continuity|resume)/i.test(
    String(input || "")
  );
}

function isStateGateEligibleFollowup(input: string | undefined): boolean {
...
function resolvePersistedHandoffFallbackReason(
  input: string | undefined,
  options: PersistedHandoffFallbackOptions = {}
): CarriedHandoffSeed["reason"] | undefined {
  if (isExplicitContinuityResumeRequest(input)) return "persisted_context_pack_explicit_resume";
  if (options.sessionBoundaryLikely) return "persisted_context_pack_state_gate";
  if (isStateGateEligibleFollowup(input)) return "persisted_context_pack_ambiguous_followup";
  return undefined;
}
...
function extractContinuityIntentInput(input: string | undefined): string {
  const text = String(input || "");
  const currentUserRequestMatch = text.match(/Current user request:\s*([\s\S]*)$/i);
  if (currentUserRequestMatch?.[1]?.trim()) return currentUserRequestMatch[1].trim();

  const transcriptUserLines = Array.from(text.matchAll(/^#\d+\s+.*?:\s*(.*)$/gm))
    .filter((match) => !/\sAigis:\s/.test(match[0]))
    .map((match) => match[1]?.trim())
    .filter(Boolean);
  if (transcriptUserLines.length > 0) return transcriptUserLines[transcriptUserLines.length - 1] || "";

  return text;
}

function isContinuityFallbackBlockedInput(input: string | undefined): boolean {
  return /(왜\s*계속|계속\s*나오|문구|footer|푸터|없애|빼라|제거|방금\s*(?:네가|보낸|답변)|근본\s*원인|구조적\s*수정|정밀\s*분석|재등장\s*금지)/i.test(
    String(input || "")
  );
}
...
function buildPersistedHandoffFallback(
  ...
  if (isContinuityFallbackBlockedInput(intentInput)) return undefined;
...
function defaultContextMeshWorkspaceDir(): string {
  return process.env.BEAI_CONTEXT_MESH_CWD || "/Users/jyp/Documents/Playground 2";
}

function defaultContextMeshBeaiBin(meshWorkspaceDir: string): string {
  const harnessBin = path.join(meshWorkspaceDir, "beai-harness-for-codex", "bin", "beai.js");
  if (fs.existsSync(harnessBin)) return harnessBin;
  return path.join(meshWorkspaceDir, "bin", "beai.js");
}

function compactContextMeshText(value: unknown, limit = 220): string {
  return String(value || "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, limit);
...
function readContextMeshHitBody(meshWorkspaceDir: string, hitPath: string, limit = 1800): { body: string; bodyLoaded: boolean } {
  const resolvedPath = resolveContextMeshHitPath(meshWorkspaceDir, hitPath);
  if (!resolvedPath) return { body: "", bodyLoaded: false };
  try {
    const stat = fs.statSync(resolvedPath);
    if (!stat.isFile() || stat.size > 1024 * 1024) return { body: "", bodyLoaded: false };
    const body = fs
...
function buildContextMeshLoadedHits(
  result: Record<string, unknown>,
  meshWorkspaceDir: string,
  limit = 4
): ContextMeshLoadedHit[] {
...
function buildContextMeshEnforcementText(block: ContextMeshEnforcementBlock | undefined): string {
  if (!block || block.hits.length === 0) return "";
  const lines = [
  ...
  block.hits.forEach((hit, index) => {
    ...
    if (hit.hitPath) lines.push(`  path: ${hit.hitPath}`);
    lines.push(`  body_loaded: ${hit.bodyLoaded ? "true" : "false"}`);
    if (hit.body) lines.push(`  body_excerpt: ${hit.body}`);
    else if (hit.excerpt) lines.push(`  fallback_excerpt: ${hit.excerpt}`);
  });
  return lines.join("\n");
}

function buildContextMeshHandoffFromResult(
  result: Record<string, unknown> | undefined,
  sessionKey: string | undefined,
  currentInput = "",
  meshWorkspaceDir = defaultContextMeshWorkspaceDir()
): CarriedHandoffSeed | undefined {
  if (!result || result.status !== "ready" || result.mustReadBeforeAnswer !== true) return undefined;
  const selectedHits = buildContextMeshLoadedHits(result, meshWorkspaceDir, 5);
  if (selectedHits.length === 0) return undefined;
  const carry = selectedHits.map(
    (hit) =>
      `${hit.tier}: ${hit.title}${hit.hitPath ? ` (${hit.hitPath})` : ""}${hit.body ? ` - ${compactContextMeshText(hit.body, 180)}` : hit.excerpt ? ` - ${hit.excerpt}` : ""}`
  );
...
function readContextMeshTurnStartResult(meshWorkspaceDir: string): Record<string, unknown> | undefined {
...
function buildContextMeshTurnStartHandoff(
  currentInput: string,
  sessionKey: string | undefined,
  options: PersistedHandoffFallbackOptions = {}
): CarriedHandoffSeed | undefined {
...
function mergeCarriedHandoffs(
  primary: CarriedHandoffSeed | undefined,
  secondary: CarriedHandoffSeed | undefined
): CarriedHandoffSeed | undefined {
  ...
  return {
    ...
    handoffState: {
      ...
      new_session_opening_message:
        primary.handoffState?.new_session_opening_message || secondary.handoffState?.new_session_opening_message,
...
function mergePlanningInput(currentInput: string, carriedHandoff: CarriedHandoffSeed | undefined): string {
  ...
  if (!prefix) return sanitizedCurrentInput;
  if (!sanitizedCurrentInput) return prefix;
...
function applyCarriedHandoff(plan: BeaiTurnPlan, carriedHandoff: CarriedHandoffSeed | undefined): BeaiTurnPlan {
  ...
  const updatedPlan = {
    ...
    judgmentTags,
    responseStrategy:
      carriedHandoff?.enforcement && plan.responseStrategy === "direct_reply" ? "structured_plan" : plan.responseStrategy,
    judgmentFrame: {
      ...plan.judgmentFrame,
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    safeOn("reply_payload_sending", async (event, ctx) => {
      ...
      const sanitizedPayloadText =
        typeof event.payload?.text === "string" && plan ? sanitizeUserFacingReply(event.payload.text, plan) : null;
