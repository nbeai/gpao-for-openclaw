...
export function buildDecisionHandleSurfaceProfile(plan) {
    ...
    if (kind === "verification")
        guidance.push("미확인 항목이 남아 있으면 결론을 강하게 닫지 말고 다음 확인 하나를 제시합니다.");
    if (kind === "judgment")
...
export function renderSessionSplitApprovalReply(plan) {
    ...
    if (handoffState) {
        ...
        if (closureHandle)
            lines.push(`마지막에 붙잡을 기준은 ${closureHandle}입니다.`);
...
export function buildSessionContinuityState(plan) {
    ...
    const closureHandle = compactText(handoffState?.closure_handle || flowState?.closureHandle || decisionHandleSurface?.primaryHandle || nextAction, 220) ||
        "다음 판단 기준을 확인합니다.";
