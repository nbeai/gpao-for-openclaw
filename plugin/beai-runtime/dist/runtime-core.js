...
function firstSentence(text) {
    ...
    if (!trimmed)
        return "사용자 요청을 정리합니다.";
    const match = trimmed.match(/^(.{1,220}?[.!?]|.{1,220}$)/);
    return (match?.[1] ?? trimmed).trim();
}
function cleanCurrentTurnInput(text) {
...
function detectFollowUpScope(input) {
...
function currentUserRequestForClassification(input) {
    const match = String(input || "").match(/Current user request:\s*([\s\S]*)$/i);
    return match?.[1]?.trim() || input;
}
function isAmbiguousPermissionContinuationRequest(input) {
...
function detectRequestedOutputShape(input) {
    if (isAmbiguousPermissionContinuationRequest(input))
        return "question";
...
function detectCurrentTarget(input) {
...
function detectExplicitConstraints(input) {
...
function detectMissingCriticalInputs(input, requestedOutputShape, followUpScope) {
    const missing = [];
...
function isCompactApprovalContinuationInput(input) {
...
export function classifyExecutionMode(input) {
    if (isAmbiguousPermissionContinuationRequest(input))
        return "planning";
    ...
    if (isCompactApprovalContinuationInput(normalized))
        return "execution";
...
export function deriveResponseStateLabel(plan) {
    if (plan.currentTurn.missingCriticalInputs.length > 0)
        return "waiting_user";
    if (plan.requiresUserConfirmation)
        return "waiting_approval";
    if (/(안\s*돼|무응답|실패|blocked|failed|error|오류)/i.test(plan.currentTurn.cleanInput) && plan.primaryClass === "diagnosis")
        return "thinking";
    const executionMode = plan.executionMode || classifyExecutionMode(plan.currentTurn.cleanInput);
    if (executionMode === "execution" || executionMode === "implementation")
        return "running";
    if (executionMode === "verification" || executionMode === "diagnosis" || executionMode === "planning")
        return "thinking";
    return "listening";
}
...
export function buildFlowStateSpine(input) {
    ...
    const currentJudgmentImpact = input.evidenceLedger.runtimeInferred
        ...
        .slice(0, 3)
        .map((summary) => ({
        type: /handoff|session/i.test(summary) ? "session_continuity" : /project/i.test(summary) ? "project_state" : "package_knowledge",
        currentJudgmentImpact: "medium",
...
function detectFlowDeliverySurface(input) {
...
function flowEvidenceFromText(text, positive, blocked, required) {
...
export function buildFlowStateEvidenceState(input) {
    ...
    const configured = flowEvidenceFromText(evidenceText, /\bconfigured\b|설정\s*(?:확인|완료)|config(?:ured)?\s*[:=]\s*(?:true|ok|verified)|BEAI load paths?:\s*[1-9]/i, /not\s*configured|설정\s*(?:없|누락|실패)|No BEAI load path/i, /설정\s*(?:필요|확인\s*필요)|configuration\s*required|config.*needs?\s*verification/i);
    const registered = flowEvidenceFromText(evidenceText, /\bregistered\b|등록\s*(?:확인|완료)|plugins?\s+list.*(?:BEAI|beai-runtime)|BEAI Runtime appears in OpenClaw plugins list/i, /not\s*registered|등록\s*(?:없|누락|실패)|BEAI Runtime not visible/i, /등록\s*(?:필요|확인\s*필요)|registration\s*required/i);
    const callable = flowEvidenceFromText(evidenceText, /\bcallable\b|호출\s*(?:가능|확인)|plugins doctor.*(?:pass|ok|통과)|hooks?.*(?:ready|ok|통과)|gateway.*(?:health|status).*(?:ok|통과)/i, /not\s*callable|호출\s*(?:불가|실패)|plugins doctor.*(?:fail|failed|실패)|hooks?.*(?:not ready|fail|실패)/i, /호출\s*(?:필요|확인\s*필요)|callable\s*required|doctor\/hooks\/tasks\/gateway\/Telegram 검증 신호/i);
    const outputVerified = flowEvidenceFromText(evidenceText, /output[_\s-]?verified|출력\s*검증\s*(?:완료|통과)|messageId["':=\s]+[^"',}\s]+|visible[_\s-]?delivery[_\s-]?verified/i, /output\s*(?:unverified|failed)|출력\s*(?:미검증|실패)|messageId\s*(?:없|missing)|visible[_\s-]?delivery.*unverified/i, /output\s*verification\s*required|출력\s*검증\s*필요|visible[_\s-]?delivery\s*required|messageId.*required/i);
    const doctor = flowEvidenceFromText(evidenceText, /doctor.*(?:pass|ok|healthy|통과|문제 없음)|BEAI Doctor.*(?:ready|verified)/i, /doctor.*(?:fail|failed|blocked|실패|blocked)|plugins-doctor-failed/i, /doctor.*(?:required|필요|확인)|닥터.*(?:필요|확인)/i);
    const release = flowEvidenceFromText(evidenceText, /release verifier.*(?:pass|ok|ready|verified)|release.*(?:verified|ready)|릴리스.*(?:검증 완료|준비 완료)/i, /release.*(?:blocked|failed|not ready)|릴리스.*(?:차단|실패|미준비)/i, /release verifier.*required|release.*(?:review|required)|릴리스.*(?:검증 필요|확인 필요)|배포.*(?:검증 필요|확인 필요)/i);
    return {
        configured,
        registered,
        callable,
...
export function buildRuntimeResponseGateProfile(input) {
    const blockIf = new Set();
    const mustPreserve = new Set();
    const mustAvoid = new Set();
    const guidance = new Set();
    const artifactFirst = input.flowState.responseRole === "artifact" || input.responseResolution.level === "artifact_first"
        ? "required"
        : "not_applicable";
    const followUpDeltaOnly = input.currentTurn.followUpScope !== "full" || input.responseInertia.requiredShift === "shorter_delta_only"
        ? "required"
        : "not_applicable";
    const visibleDelivery = input.flowState.deliverySurface.visibleDelivery === "unverified" || input.flowState.deliverySurface.visibleDelivery === "required"
        ? "required"
        : "not_applicable";
    ...
    for (const item of input.conversationQualityGuard.mustPreserve)
        mustPreserve.add(item);
    for (const item of input.conversationQualityGuard.mustAvoid)
        mustAvoid.add(item);
    for (const item of input.conversationQualityGuard.regressionChecks.slice(0, 4))
        blockIf.add(item);
    return {
        version: "0.1",
        mode: "guide_only",
...
export function classifyToolCallRisk(input) {
    ...
    if (reasons.size === 0)
        reasons.add("no elevated risk signal detected");
    const riskLevel = riskRank >= 2 ? "high" : riskRank === 1 ? "medium" : "low";
    return {
        toolName,
        riskLevel,
...
export function classifyToolResultSemantics(input) {
    ...
    const reasons = new Set();
    const state = hasError ? "failed" : hasResult ? "succeeded" : "unknown";
    if (hasError)
        reasons.add("tool returned an error");
    else if (hasResult)
        reasons.add("tool returned a result");
    else
        reasons.add("tool completed without an observable result payload");
...
function shouldSanitizePersistedToolResult(text) {
    const normalized = text.toLowerCase();
    if (text.length > 6000)
        return true;
    return (/\[system\]\s*your previous turn was interrupted/i.test(text) ||
...
function sanitizeToolResultBlock(block, replacement) {
...
export function sanitizePersistedToolResultMessage(message, toolName) {
...
export function buildCurrentTurnPacket(rawInput) {
...
function inferConstraintsFromPrompt(text) {
    const results = new Set();
    if (text.includes("충돌"))
        results.add("기존 OpenClaw 환경과 충돌시키지 않습니다.");
    if (text.includes("덮어쓰"))
        results.add("기존 설정을 임의로 덮어쓰지 않습니다.");
    if (text.includes("승인"))
        results.add("고위험 변경은 승인 전까지 실행하지 않습니다.");
    if (text.includes("토큰"))
        results.add("불필요한 장문 응답과 토큰 낭비를 줄입니다.");
    if (text.includes("실전"))
        results.add("설명보다 실제 실행 가능한 결과를 우선합니다.");
    return Array.from(results);
}
function defaultDeliverables(mode) {
    if (mode === "handoff") {
        return [
...
function defaultAcceptanceChecks(mode) {
    if (mode === "handoff") {
        return [
...
function mapResponseStrategy(plan) {
...
function parseSessionContextUsagePct(prompt) {
    const patterns = [
...
function buildOpenClawWorkOrder(sharedPlan, prompt, constraints) {
    if (sharedPlan.mode !== "handoff")
        return undefined;
    return {
        taskType: "execution",
...
function mapResponseRole(sharedPlan, currentTurn) {
    if (isCompactApprovalContinuationInput(currentTurn.cleanInput))
        return "work_order";
    if (sharedPlan.requiresUserConfirmation || sharedPlan.turnJudgment === "defer")
        return "defer";
    if (currentTurn.missingCriticalInputs.length > 0 || sharedPlan.turnJudgment === "clarify")
        return "clarify";
    if (sharedPlan.mode === "handoff" || sharedPlan.turnJudgment === "execute")
        return "work_order";
    if (sharedPlan.primaryClass === "diagnosis" || sharedPlan.turnJudgment === "diagnose")
        return "diagnosis";
    if (sharedPlan.primaryClass === "planning" || sharedPlan.primaryClass === "package_readiness")
        return "plan";
    if (sharedPlan.primaryClass === "verification" || sharedPlan.turnJudgment === "verify")
        return "judgment";
...
function buildAskOneQuestion(missing) {
...
export function buildJudgmentFrame(currentTurn, sharedPlan) {
    const confirmed = new Set();
    const assumptionCandidates = new Set();
    const unknown = new Set(currentTurn.missingCriticalInputs);
    if (currentTurn.cleanInput)
        confirmed.add(`current_input: ${currentTurn.cleanInput}`);
    if (currentTurn.requestedOutputShape)
        confirmed.add(`requested_output_shape: ${currentTurn.requestedOutputShape}`);
    if (currentTurn.followUpScope && currentTurn.followUpScope !== "full")
        confirmed.add(`follow_up_scope: ${currentTurn.followUpScope}`);
    if (currentTurn.currentTarget)
        confirmed.add(`current_target: ${currentTurn.currentTarget}`);
    if (sharedPlan.primaryClass)
        confirmed.add(`primary_class: ${sharedPlan.primaryClass}`);
    if (sharedPlan.mode)
        confirmed.add(`mode: ${sharedPlan.mode}`);
    for (const constraint of currentTurn.explicitConstraints)
        confirmed.add(`constraint: ${constraint}`);
...
export function buildResponseResolution(currentTurn, sharedPlan, judgmentFrame, conversationScene) {
...
function detectPreviousResponseSurface(reply) {
...
function wantsSameStructure(input) {
    const normalized = input.trim();
    const hasSameStructureSignal = /(같은\s*(형식|구조|톤|방식)|위\s*(형식|구조)\s*유지|방금처럼|same\s*(format|structure|style))/i.test(normalized);
    if (!hasSameStructureSignal)
        return false;
    const negatesReuse = /(같은\s*(형식|구조|톤|방식)[^.!?\n]{0,16}(쓰지|하지|말고|말아|아니|빼고|금지)|방금처럼[^.!?\n]{0,16}(쓰지|하지|말고|말아|아니|빼고|금지)|not\s+(same|the same)|do\s+not\s+use\s+(the\s+)?same)/i.test(normalized);
    return !negatesReuse;
}
function isShortSceneContinuationInput(input) {
    const normalized = input.replace(/\s+/g, " ").trim();
...
function inferSharedCommitmentsFromScene(sourceText) {
...
function inferPendingNextActionsFromScene(sourceText) {
...
function inferCurrentSceneFromPatch(patch, sourceText) {
...
export function buildConversationSceneContinuityProfile(input) {
    ...
    const utteranceRole = repairInput
...
function inferInputMaturity(input, currentTurn, sharedPlan) {
    ...
    if (currentTurn.requestedOutputShape === "execution" ||
...
function inferInputLevelNeed(input, currentTurn, sharedPlan, maturity) {
    const text = input.replace(/\s+/g, " ").trim();
    const hasFailureContext = /(오류|무응답|안\s*돼|멈췄|반복|느려|끊|같은\s*답변|고장|실패|stuck|failed|error|telegram|gateway|텔레그램|게이트웨이)/i.test(text);
    if (hasFailureContext && /(원인|문제|진단|분석|찾아|왜|고쳐|수리|닥터|doctor)/i.test(text))
        return "diagnosis";
    ...
    if (hasReadOnlyAutomationInspectionSignal(text) || hasReadOnlyTelegramStatusSignal(text))
        return "verification";
    if (hasReadOnlyNotionSignal(text))
        return /(요약|정리|summarize|summary)/i.test(text) ? "research" : "verification";
    if (hasOperationalDebtSignal(text))
        return "problem_framing";
    if (hasSkillDescriptionArtifactSignal(text))
        return "artifact";
    if (hasDocsOnlyReleaseArtifactSignal(text))
        return "artifact";
    if (/(전달(?:할|용).{0,24}(?:리포트|보고서|문서).{0,18}(?:만|작성|만들|써|정리)|(?:리포트|보고서|문서).{0,24}전달용|(?:codex|코덱스).{0,28}(?:전달|보낼).{0,24}(?:리포트|보고서|문서).{0,18}(?:만|작성|만들|써|정리))/i.test(text))
        return "artifact";
    if (hasRetryPolicySignal(text))
        return "automation_readiness";
    if (hasDestructiveRewriteSignal(text) || hasOpenClawCoreMutationSignal(text))
        return "execution";
    if (hasMedicalSafetySignal(text))
        return "verification";
    ...
    if (hasAgentCandidateSignal(text))
        return "workflow_design";
    if (hasExternalWriteSignal(text) || hasRollbackSignal(text) || hasIssueLedgerWriteSignal(text) || hasVersionMutationSignal(text))
        return "execution";
    if (hasAutomaticMemorySignal(text))
        return "automation_readiness";
    if (currentTurn.requestedOutputShape === "artifact" ||
        /(써줘|작성|초안|문서|문구|메시지|인사말|카피|제목|공지|메일|안내문|표로|체크리스트)/i.test(text) ||
        /(문장|문구|안내문|메일|공지|메시지).{0,24}(바꿔|수정|만들|작성|써|다시)/i.test(text))
        return "artifact";
    if (hasAppProductSignal(text))
        return maturity === "unformed" || maturity === "goal_seeking" ? "first_success_path" : "execution";
    if (/(검색|조사|자료|리서치|근거|사례|공신력|웹|latest|최신)/i.test(text))
        return "research";
    if (/(점검|검증|확인|테스트|상태|doctor|닥터|로그|원인|오류|무응답|안\s*돼)/i.test(text) || sharedPlan.primaryClass === "verification")
        return "verification";
    if (sharedPlan.primaryClass === "diagnosis" || /(왜|문제|진단|분석|원인|불만|고충)/i.test(text))
        return "diagnosis";
    ...
    if (maturity === "unformed")
        return "orientation";
    if (maturity === "pain_signal")
        return "problem_framing";
    if (maturity === "goal_seeking")
        return "first_success_path";
    if (maturity === "specific_execution" || maturity === "expert_directive")
        return "execution";
    return "first_success_path";
}
function inputLevelSkillFamilies(need, input) {
    const families = new Set();
    const routingNegated = hasExplicitRoutingNegationSignal(input);
    if (need === "orientation" || need === "first_success_path")
        families.add("ai-native-journey-guide");
    if (need === "problem_framing")
        families.add("owner-intake-router");
    if (need === "workflow_design" && !routingNegated)
        families.add("ax-first-automation-planner");
    if (need === "automation_readiness") {
        families.add("skill-agent-cron-router");
        families.add("automation-readiness-check");
    }
    if (need === "diagnosis" || need === "verification" || /비아이\s*닥터|beai doctor|openclaw|telegram|gateway/i.test(input))
        families.add("beai-doctor");
    if (hasSkillLifecycleSignal(input) && !routingNegated)
        families.add("skill-creator");
    if (hasScheduledWorkSignal(input))
        families.add("automation-readiness-check");
    if (hasAgentCandidateSignal(input) && !routingNegated)
        families.add("skill-agent-cron-router");
    if (hasAppProductSignal(input))
        families.add("beai-development-steward");
    if (hasVersionMutationSignal(input))
        families.add("beai-development-steward");
    if (hasIssueLedgerWriteSignal(input))
        families.add("beai-doctor");
    return Array.from(families).slice(0, 4);
}
function inferHumanJourneyStage(input, maturity, need) {
    if (/(오류|안\s*돼|무응답|멈췄|반복|느려|끊|stuck|failed|error|telegram|gateway|게이트웨이|텔레그램)/i.test(input)) {
        return "error_trust_break";
    }
    if (/(설치.*(끝|완료|했)|깔았|installed).*(뭐|어떻게|이제|막막|모르겠)/i.test(input))
        return "post_install_blank";
    if (hasOperationalDebtSignal(input))
        return "operational_debt";
    if (need === "automation_readiness" || /(매일|자동화|cron|크론|정기|반복|예약\s*(?:실행|발송|등록|작업)|모니터)/i.test(input))
        return "automation_desire";
    if (need === "execution" || /(진행|수정|적용|삭제|보내|실행|배포|설치)/i.test(input))
        return "pre_execution_control";
    if (maturity === "expert_directive")
        return "expert_expansion";
    if (/(어떻게 말|이렇게 말|뭐라고|프롬프트|질문하면|입력)/i.test(input))
        return "first_input_anxiety";
    if (/(선택|비교|가능|옵션|길|방법|방향|장단점|기준)/i.test(input))
        return "choice_criteria";
    if (maturity === "unformed" || need === "orientation")
        return "possibility_awareness";
    if (maturity === "goal_seeking" || need === "workflow_design")
        return "possibility_exploration";
    return "choice_criteria";
}
function hasOperationalDebtSignal(text) {
...
function hasRiskyArtifactSignal(text) {
    const normalized = text.replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
...
function hasSimpleArtifactRequest(text, primaryNeed) {
    const normalized = text.replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
    const artifactSignal = primaryNeed === "artifact" ||
...
function hasInterpersonalRetaliationSignal(text) {
    const normalized = text.replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
...
export function buildInputLevelCompanionProfile(input) {
    ...
    const userBurden = inputMaturity === "unformed" || /(막막|어렵|답답|포기|현타|도대체|모르겠)/i.test(text)
        ? "high"
        : inputMaturity === "pain_signal" || input.currentTurn.missingCriticalInputs.length > 0
            ? "medium"
            : "low";
    const cognitiveLoadLevel = userBurden === "high" || /(복잡|헷갈|어렵|모르겠|막막|불안|짜증|현타|너무 많)/i.test(text)
        ? "high"
        : inputMaturity === "expert_directive" || text.length > 220 || input.currentTurn.missingCriticalInputs.length > 0
            ? "medium"
            : "low";
    const choiceOwnershipRisk = simpleArtifactRequest
        ? "low"
        : interpersonalRetaliationSignal || /(정해줘|골라줘|추천만|하나만|뭐가 맞아|답을 줘|시키는 대로|알아서)/i.test(text)
            ...
            : primaryNeed === "orientation" || primaryNeed === "workflow_design" || primaryNeed === "automation_readiness" || /(선택|결정|고민|방향|가능)/i.test(text)
                ? "medium"
                : "low";
    const trustCalibrationNeed = interpersonalRetaliationSignal || journeyStage === "error_trust_break" || primaryNeed === "diagnosis" || primaryNeed === "verification"
        ? "strong"
        : riskyArtifactSignal || primaryNeed === "automation_readiness" || primaryNeed === "execution" || choiceOwnershipRisk !== "low"
            ? "light"
            : "none";
    const possibleWorldsNeeded = simpleArtifactRequest
        ? "none"
        : primaryNeed === "orientation" || journeyStage === "possibility_awareness" || journeyStage === "possibility_exploration"
            ...
            : choiceOwnershipRisk !== "low" || primaryNeed === "problem_framing" || journeyStage === "choice_criteria"
                ? "light"
                : "none";
    const explanationDepth = inputMaturity === "expert_directive"
        ? "technical"
        : cognitiveLoadLevel === "high"
            ...
            : possibleWorldsNeeded === "structured"
                ? "structured"
                : "minimal";
    const controlBoundaryNeeded = !readOnlyStatusSignal &&
    ...
    const responsePosture = primaryNeed === "artifact"
        ? "produce"
            ...
            : primaryNeed === "execution"
                ...
                : primaryNeed === "orientation"
                    ...
                    : primaryNeed === "problem_framing"
                        ? "frame"
                        : "guide";
    const questionBudget = userBurden === "high" ? 1 : input.currentTurn.missingCriticalInputs.length > 0 ? 1 : responsePosture === "guide" ? 2 : 0;
    const recommendedSkillFamilies = inputLevelSkillFamilies(primaryNeed, text);
    const firstSafeStep = primaryNeed === "orientation"
    ...
    if (possibleWorldsNeeded !== "none")
        guidance.push("AI가 선택지를 대신 좁히기보다 가능한 길, 조건, 비용, 위험을 이해 가능한 구조로 펼칩니다.");
    if (choiceOwnershipRisk !== "low")
        guidance.push("추천은 가능하지만 사용자의 선택권과 선택 이후 부담을 AI가 대신 소유하지 않습니다.");
    if (riskyArtifactSignal)
        guidance.push("외부·고객·계약·의료성 문구는 짧아도 톤, 사실성, 승인 경계를 보정합니다.");
    if (interpersonalRetaliationSignal)
        guidance.push("대인관계 보복·망신 요청에서는 감정의 타당성과 실행 행동을 분리하고, 사용자에게 동조해 해로운 행동을 정당화하지 않습니다.");
    if (hasMedicalSafetySignal(text))
        guidance.push("의약품·복용 판단은 이 대화만으로 직접 허용하지 말고, 처방 기준·약사/의사 확인·응급 증상 여부를 먼저 분리합니다.");
    ...
    if (trustCalibrationNeed === "strong")
        guidance.push("오류나 검증 상황에서는 확인된 것, 처리 중인 것, 실패한 것을 분리해 신뢰를 보정합니다.");
...
function classifyResponseInertiaRelation(currentTurn) {
    const input = currentTurn.cleanInput;
    const normalized = input.replace(/\s+/g, " ").trim();
    if (!normalized)
        return "unknown";
    if (wantsSameStructure(normalized))
        return "continues_previous";
    if (currentTurn.requestedOutputShape === "artifact" || currentTurn.requestedOutputShape === "work_order")
        return "artifact_request";
    if (/(써줘|작성해|초안|문구|공지|메일|메시지|안내문|다시\s*만들|문서로|artifact|draft|write|compose)/i.test(normalized))
        return "artifact_request";
    if (/(^|\s)(아니\s*(야|요|다|고|\.|,|$)|그건\s*아니|그게\s*아니라|틀렸|다시\s*봐|오해|잘못\s*봤|not\s+that|wrong)/i.test(normalized))
        return "rejects_previous";
    if (/(근데|다만|추가로|보정|수정|바꿔|변경|새\s*정보|사실은|실제로는|정확히는|actually|instead)/i.test(normalized))
        return "modifies_previous";
    if (/(별도|분리해서|separate\s*(issue|context|problem)?)/i.test(normalized))
        return "separate_issue";
    if (/(다른\s*(얘기|주제|문제)|이번엔|이번에는|새로|new\s+topic)/i.test(normalized))
        return "new_direction";
    if (/(결론만|짧게|한\s*줄|그래서\?|so\?|bottom\s*line|간단히)/i.test(normalized) || currentTurn.followUpScope !== "full")
        return "simple_followup";
    if (normalized.length <= 36)
        return "simple_followup";
    return "continues_previous";
}
function chooseResponseInertiaShift(relation, currentTurn) {
    if (wantsSameStructure(currentTurn.cleanInput))
        return "none";
    if (relation === "artifact_request")
        return "produce_artifact";
    if (relation === "rejects_previous")
        return "reframe";
    if (relation === "modifies_previous" || relation === "simple_followup")
        return "shorter_delta_only";
    if (relation === "new_direction")
        return "answer_directly";
    if (relation === "separate_issue")
        return "separate_contexts";
    if (currentTurn.missingCriticalInputs.length > 0)
        return "ask_one_new_variable";
...
export function buildResponseInertiaProfile(input) {
    ...
    const previousStructureRisk = previousSurface.hasNumberedStructure || previousSurface.hasSectionHeadings || previousSurface.hasStrongConclusionFormula || previousSurface.endsWithQuestion;
    ...
    const requiredShift = sceneExecutionContinuation && relation !== "artifact_request"
        ? "close_without_question"
        : chooseResponseInertiaShift(relation, input.currentTurn);
    const previousResponseReuseRisk = explicitSameStructure
        ? "low"
    ...
    if (requiredShift === "shorter_delta_only")
        guidance.push("이전 설명 전체를 반복하지 말고 바뀐 지점만 짧게 답합니다.");
    if (requiredShift === "reframe")
        guidance.push("사용자 보정이 들어오면 기존 판단을 방어하지 말고 보정된 전제로 재정렬합니다.");
    if (requiredShift === "produce_artifact")
        guidance.push("산출물 요청이면 분석을 반복하지 말고 결과물을 먼저 둡니다.");
    if (requiredShift === "answer_directly")
        guidance.push("새 방향이면 이전 프레임을 끌고 오지 말고 현재 요청에 직접 답합니다.");
    if (requiredShift === "ask_one_new_variable")
        guidance.push("정말 필요한 경우에만 새 변수 하나를 묻습니다.");
...
export function buildJudgmentSharpnessProfile(plan) {
    const toolVerifiedCount = plan.evidenceLedger.toolVerified.filter((item) => item.trim()).length;
    const needsVerificationCount = plan.evidenceLedger.needsVerification.length +
    ...
    const asksForJudgment = plan.responseResolution.level === "judgment_deep" ||
        plan.responseResolution.level === "execution_check" ||
        plan.judgmentFlow.domain === "release_runtime";
    let claimStrength = "bounded";
    ...
    let uncertaintyAction = "state_verified";
    ...
    const shouldAskNewQuestion = uncertaintyAction === "ask_one_check" &&
    ...
    if (uncertaintyAction === "avoid_completion_claim")
        guidance.push("완료, 적용됨, 자동화됨, 프로덕션급 같은 강한 표현은 도구 검증 뒤에만 씁니다.");
    if (uncertaintyAction === "separate_verified_from_unknown")
        guidance.push("확인된 것과 남은 확인을 한 문단 안에서 섞지 말고 분리합니다.");
    if (uncertaintyAction === "verify_first")
        guidance.push("판단을 강하게 닫기 전에 확인 경로 또는 검증 결과를 먼저 제시합니다.");
    if (shouldAskNewQuestion)
        guidance.push("질문이 필요하면 새 변수 하나만 묻습니다.");
    const guardNote = claimStrength === "verified"
...
export function buildSurfaceFlowProfile(currentTurn, sharedPlan, judgmentFrame, responseResolution) {
    const input = currentTurn.cleanInput;
    const asksState = sharedPlan.primaryClass === "verification" ||
        sharedPlan.turnJudgment === "verify" ||
        /(상태|점검|확인|검증|status|doctor|hooks|tasks|logs)/i.test(input);
    const asksJudgment = sharedPlan.primaryClass === "diagnosis" ||
    ...
    const artifactFirst = !asksJudgment &&
    ...
    if (asksJudgment)
        avoid.push("abstract conclusion before the user's concrete situation");
    if (artifactFirst)
        avoid.push("long preface before the requested artifact");
    if (asksState)
        avoid.push("complete claim before verified status");
    return {
        mode: "guide_only",
        rewriteOutput: false,
...
export function buildDecisionHandleSurfaceProfile(plan) {
    ...
    const closeStyle = isArtifact
        ...
        : hasRemainingChecks
            ...
            : kind === "next_action" || kind === "handoff"
                ? "next_action_first"
                : "decision_first";
    const nextVerification = plan.judgmentFlow.unresolvedChecks[0] || plan.realitySignalProfile.needsVerification[0];
    const primaryHandle = kind === "artifact"
    ...
    if (kind === "verification")
        guidance.push("미확인 항목이 남아 있으면 결론을 강하게 닫지 말고 확인이 필요한 범위를 본문 안에서 자연스럽게 분리합니다.");
    if (kind === "judgment")
        guidance.push("판단 요청에서는 선택지를 늘리기보다 현재 가장 중요한 판단 기준 하나를 남깁니다.");
    if (kind === "artifact")
        guidance.push("산출물 요청에서는 사용자가 바로 쓸 결과물을 먼저 제공합니다.");
    return {
        version: "0.4.8",
        mode: "guide_only",
        kind,
...
export function buildConversationalRhythmProfile(plan) {
    ...
    const structure = artifact
        ...
        : state || verification
            ...
            : execution
                ...
                : "plain_paragraphs";
    const companionStance = frustration
        ? "steady"
        : verification || state
            ? "careful"
            : execution
    ...
    if (artifact)
        avoid.push("preface before the requested artifact");
    if (verification)
        avoid.push("certainty beyond verified evidence");
    if (vulnerableConcern)
        avoid.push("premature summary before the user's situation is held");
    const guidance = [
        "사용자의 현재 정서와 요청 모드를 먼저 맞춘 뒤 필요한 구조를 씁니다.",
        "자연스러운 대화 흐름을 유지하되, 근거와 확인 경계를 흐리지 않습니다."
    ];
    if (frustration)
        guidance.push("분노나 답답함이 있으면 방어적으로 설명하지 말고 문제를 정확히 다시 잡고 바로 좁혀 갑니다.");
    if (vulnerableConcern)
        guidance.push("고민 상담에서는 사용자의 현실을 먼저 붙잡고, 결론은 기준과 다음 행동으로 천천히 닫습니다.");
    if (artifact)
        guidance.push("산출물 요청에서는 말보다 결과물을 먼저 줍니다.");
    if (verification)
        guidance.push("검증 요청에서는 확인된 것과 아직 확인하지 않은 것을 리듬상 분리합니다.");
    return {
        version: "0.4.8",
        mode: "guide_only",
        openingMove,
...
function hasScheduledWorkSignal(text) {
    if (hasReadOnlyAutomationInspectionSignal(text) || hasExplicitRoutingNegationSignal(text))
        return false;
    return SCHEDULE_AUTOMATION_PATTERN.test(text);
}
function hasExternalWriteSignal(text) {
    if (hasReadOnlyNotionSignal(text) || hasReadOnlyTelegramStatusSignal(text))
        return false;
    if (hasDraftOnlyExternalArtifactSignal(text))
        return false;
...
function hasDraftOnlyExternalArtifactSignal(text) {
...
function hasSkillLifecycleSignal(text) {
    if (hasExplicitRoutingNegationSignal(text) || hasSkillDescriptionArtifactSignal(text))
        return false;
...
function hasAgentCandidateSignal(text) {
...
function hasAppProductSignal(text) {
...
function hasAuthDataPaymentDeploySignal(text) {
...
function hasRollbackSignal(text) {
    return /(롤백|되돌려|원복|복구|이전\s*버전으로|rollback|revert|restore)/i.test(text);
}
function hasAutomaticMemorySignal(text) {
...
function hasReadOnlyTelegramStatusSignal(text) {
...
function hasReadOnlyNotionSignal(text) {
...
function hasExplicitRoutingNegationSignal(text) {
...
function hasSkillDescriptionArtifactSignal(text) {
    return /(스킬\s*(?:설명|소개|문구|카피).{0,24}(?:다듬|수정|작성|써|바꿔)|스킬\s*설명만|스킬\s*소개\s*문구)/i.test(text);
}
function hasDocsOnlyReleaseArtifactSignal(text) {
...
function hasOpenClawCoreMutationSignal(text) {
...
function hasSameScopeApprovalContinuationSignal(text) {
...
function hasMedicalSafetySignal(text) {
    return /(약|의약품|복용|먹어도\s*돼|먹어도\s*되|처방|부작용|응급|진통제|항생제|medicine|medication|dose|dosage)/i.test(text);
}
function hasDurableProcedureSignal(text) {
    return /(?:앞으로|매번|항상).{0,40}(?:이\s*절차|이\s*형식|이\s*방식|절차로|형식으로|방식으로).{0,40}(?:정리|처리|작성|진행|해줘)/i.test(text);
}
function hasBusinessOperationsSignal(text) {
    const localOpsSignal = /(외식|식당|음식점|자영업|사업장|점포|매장|마느|엘리펀트|파찌|백화점|주방|홀|풀타임|직원|인력|코어\s*인력|오너|대표|몸빵|현장\s*투입|가족\s*시간|법인|법인\s*전환|종소세|법인세|세금)/i.test(text);
    const revenueWithOpsContext = /(매출|순이익|영업이익|마진)/i.test(text) &&
        /(직원|인력|세금|현금|대표|오너|현장|매장|점포|사업장|가족|법인|운영|시스템화)/i.test(text);
    return localOpsSignal || revenueWithOpsContext;
}
function hasApprovalFrustrationSignal(text) {
...
export function buildConversationQualityGuardProfile(plan) {
    const input = plan.currentTurn.cleanInput;
    const fixture = /(김제|집터|시골집|고향집|아버지|부모님|증조|상속|물려받|농막|농촌|귀촌|전원주택|대지|토지|건축|상시\s*거주|체류)/i.test(input)
        ? "family_asset"
        : hasBusinessOperationsSignal(input)
            ...
            : hasApprovalFrustrationSignal(input)
                ...
                : plan.decisionHandleSurface.kind === "artifact"
                    ? "artifact_request"
                    : "general";
    const qualityTarget = fixture === "artifact_request"
        ? "usable_artifact"
...
export function buildHumanCompanionQualityProfile(plan) {
    const input = plan.currentTurn.cleanInput;
    const reduceBy = new Set();
    const mustDo = new Set();
    const mustAvoid = new Set();
    const regressionChecks = new Set();
    ...
    const userExperienceTarget = plan.conversationQualityGuard.qualityTarget === "usable_artifact" || plan.flowState.responseRole === "artifact"
        ? "usable_output"
        : plan.inputLevelCompanion.trustCalibrationNeed === "strong"
            ...
            : plan.conversationSceneContinuity.sceneStatus === "active_scene"
                ...
                : plan.inputLevelCompanion.primaryNeed === "execution" || plan.inputLevelCompanion.primaryNeed === "verification"
                    ? "clear_next_action"
                    : "feel_understood";
    const stance = userExperienceTarget === "usable_output"
        ? "act_then_explain"
        : userExperienceTarget === "recover_trust"
            ...
            : plan.currentTurn.missingCriticalInputs.length > 0
                ? "clarify_then_act"
                : "mirror_then_move";
    const longConversationRisk = plan.conversationSceneContinuity.sceneStatus === "active_scene" && plan.responseInertia.previousResponseReuseRisk !== "low"
        ? "high"
        : plan.conversationSceneContinuity.sceneStatus === "active_scene" || input.length > 700
            ? "medium"
            : "low";
    const decisionReturn = plan.flowState.responseRole === "artifact"
        ? "바로 쓸 산출물"
...
export function buildClassificationFailSoftProfile(plan) {
    const input = plan.currentTurn.cleanInput;
    const weakSignals = [];
    if (/(가족|부모|아버지|어머니)/i.test(input))
        weakSignals.push("family_word");
    if (/(반복|매일|매주|자동화|스케줄|예약)/i.test(input))
        weakSignals.push("repetition_or_schedule_word");
    if (/(승인|approval|approve|deny|allow-once|allow-always)/i.test(input))
        weakSignals.push("approval_word");
    if (/(매출|가격|수익|고객\s*이탈|전환율)/i.test(input))
        weakSignals.push("revenue_or_growth_word");
    const signalBasis = [];
...
function addRealitySignal(signals, signal) {
    if (!signal?.text?.trim())
        return;
    const key = `${signal.label}:${signal.text}:${signal.source}`;
    if (signals.some((existing) => `${existing.label}:${existing.text}:${existing.source}` === key))
        return;
    signals.push(signal);
}
function hasStatusEvidence(text) {
...
export function buildRealitySignalMap(currentTurn, sharedPlan, constraints, judgmentFrame) {
    const signals = [];
...
export function buildEvidenceLedger(signalMap) {
    return {
...
export function buildRealitySignalProfile(signalMap, ledger) {
    ...
    const assumptions = signalMap.signals
        ...
        .map((signal) => signal.text);
    const needsVerification = Array.from(new Set([
        ...ledger.needsVerification,
        ...signalMap.signals
            .filter((signal) => signal.realityClass === "needs_verification")
            .map((signal) => signal.text)
    ]));
    let claimPosture = "can_state";
    if (needsVerification.length > 0 && observedSignals.length > 0)
        claimPosture = "say_observed_only";
    else if (needsVerification.length > 0)
        claimPosture = "ask_or_verify_first";
    else if (runtimeInferences.length > 0 || assumptions.length > 0)
        claimPosture = "state_with_boundary";
...
function strongestSignalStrength(signalMap) {
    if (signalMap.strongSignals.length > 0)
        return "strong";
    if (signalMap.mediumSignals.length > 0)
        return "medium";
    if (signalMap.weakSignals.length > 0)
        return "weak";
    return "unknown";
}
export function checkClaimStrength(text, plan) {
    const allowedClaimStrength = plan ? strongestSignalStrength(plan.realitySignalMap) : "unknown";
    const reasons = [];
    const hasStrongCompletionClaim = /(완료(?:했습니다|됐습니다|되었습니다|로 볼 수 있습니다)|해결(?:했습니다|됐습니다)|확실(?:합니다|히)|반드시|원인은\s*.+입니다)/i.test(text) ||
    ...
    return {
        ...
        reasons,
        rewriteInstruction: reasons.length > 0
            ? "완료/해결/원인 단정 대신 확인된 범위와 아직 확인되지 않은 범위를 분리합니다."
            : undefined
    };
}
export function checkSourceBoundary(text, plan) {
    const riskyPhrases = [];
    ...
    return {
        ...
        riskyPhrases,
        rewriteInstruction: riskyPhrases.length > 0
            ? "사용자가 제공한 자료라고 단정하지 말고, 현재 확인 가능한 입력 또는 별도 확인이 필요한 자료로 표현합니다."
            : undefined
    };
}
function softenOverclaimingLanguage(text, plan) {
...
function hasInterpersonalSycophancyRisk(text) {
    const normalized = text.replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
...
function softenInterpersonalSycophancyLanguage(text) {
    if (!hasInterpersonalSycophancyRisk(text))
        return text;
    let rewritten = text
...
function hasHighStakesSurfaceContext(plan) {
    if (!plan)
        return false;
    const input = plan.currentTurn.cleanInput;
    return (plan.inputLevelCompanion.trustCalibrationNeed === "strong" ||
...
function softenUnverifiedActionAndHighStakesClaims(text, plan) {
    if (!hasHighStakesSurfaceContext(plan))
        return text;
    let rewritten = text;
...
function softenChoiceDominationLanguage(text, plan) {
    if (!plan || plan.inputLevelCompanion.choiceOwnershipRisk === "low")
        return text;
    if (!/(무조건|정답은|볼\s*필요\s*없|그냥\s+.+로\s+가|하나만\s*보면|선택지는\s*없)/i.test(text))
        return text;
    let rewritten = text
...
function pushSkillCandidate(candidates, candidate) {
...
function inferSkillRoutingAction(input, score) {
    if (input.requiresUserConfirmation || input.riskLevel === "high")
        return "needs_user_approval";
    if (score >= 70 && input.mode !== "conversation")
        return "recommend_use";
    return "hold_candidate";
}
function normalizeSkillRoutingText(value) {
    let text = value || "";
    ...
    return text
        ...
        if (!normalized)
            return false;
        if (/^스킬\s*라이브\s*테스트/i.test(normalized))
            return false;
        if (/아래\s*사용자\s*발화/i.test(normalized))
            return false;
        if (/마지막에\s*사용(?:한|된)\s*스킬/i.test(normalized))
            return false;
        if (/사용(?:한|된)\s*스킬(?:과|을)?/i.test(normalized))
            return false;
        if (/판단\s*기준(?:을|과)?\s*짧게\s*표시/i.test(normalized))
            return false;
        return true;
    })
        .join("\n")
...
function isMetaOnlySkillSignal(text) {
    const stripped = text
...
function hasExplicitSkillCreationSignal(text) {
...
function hasWorkflowCandidateSignal(text) {
    return /(반복(?:되는)?\s*(?:작업|업무|일)|절차화|워크플로(?:우)?\s*(?:후보|설계|정리)|workflow\s*(?:candidate|design|plan)|수동\s*실행|manual\s*(?:test|workflow))/i.test(text);
}
function uniqueOperatingRiskFamilies(values) {
    return Array.from(new Set(values));
}
function includesOperatingSignal(text, pattern) {
    return pattern.test(text);
}
export function classifyOperatingRiskFamilies(input) {
    ...
    const installInspectionOnly = lowRiskPreparation &&
    ...
    if (input.toolRisk?.riskLevel === "high") {
        for (const reason of input.toolRisk.reasons) {
            if (/external|message delivery/i.test(reason))
                families.push("external_send");
            if (/destructive|deletion|removal/i.test(reason))
                families.push("delete");
            if (/scheduler|service|gateway|plugin lifecycle/i.test(reason))
                families.push("restart");
            if (/sensitive derived path/i.test(reason))
                families.push("permission_change");
        }
    }
    return uniqueOperatingRiskFamilies(families);
}
export function classifyOperatingStateHygieneClasses(input) {
    const text = String(input.currentInput || "").toLowerCase();
    const classes = [];
    const push = (item) => {
    ...
    if (/(memory|메모리|기억|장기\s*기억|기억\s*후보)/i.test(text))
        push("memory");
    if (/(agreement|합의|원칙|기준|accepted\s*rule|결정\s*기준)/i.test(text))
        push("agreement");
    if (/(project\s*state|프로젝트\s*상태|현재\s*기준|rollback|baseline|버전\s*기준)/i.test(text))
        push("project_state");
    if (/(session|세션|continuity|이어|handoff|압축|compaction)/i.test(text))
        push("session_continuity");
    if (/(task|queued|running|historical\s*issue|작업\s*상태|태스크)/i.test(text))
        push("task_history");
    if (/(transcript|대화창|표면|오염|residue|내부\s*문구|system\s*(message|request)|시스템\s*(요청|문구|메시지))/i.test(text))
        push("transcript_residue");
    if (/(tool\s*failure|도구\s*실패|command\s*failed|error|오류|실패)/i.test(text))
        push("tool_failure_residue");
    if (!hasSameScopeApprovalContinuationSignal(text) && /(approval|승인|allow-once|deny|plugin approval|codex_command_approval)/i.test(text))
        push("approval_residue");
    return classes;
}
export function classifyOperatingDelegationCandidate(input) {
...
export function classifyOperatingDelegationLevel(input) {
    const candidate = classifyOperatingDelegationCandidate(input).candidate;
    const riskFamilies = classifyOperatingRiskFamilies(input);
    const strongRisk = riskFamilies.some((family) => ["external_send", "delete", "payment", "browser_submit", "device_action", "permission_change", "durable_memory_write"].includes(family));
    if (candidate === "do_not_do" || strongRisk)
        return "L6_high_risk_strong_approval";
    if (candidate === "automation_candidate" || riskFamilies.includes("cron_creation"))
        return "L5_repeated_automation_candidate";
    if (candidate === "approval_required")
        return "L4_execute_after_approval";
    if (candidate === "workflow_candidate" || candidate === "agent_candidate")
        return "L3_internal_proposal";
    if (candidate === "skill_candidate")
        return "L2_review_or_organize";
    if (/(초안|draft|문안|제안문)/i.test(input.currentInput))
        return "L1_draft_only";
    return "L0_answer_only";
}
function hasLowRiskPreparationSignal(text) {
    return /(상태\s*점검|점검|진단|doctor|check|verify|검증|dry[-\s]?run|드라이런|읽|확인|로그|목록|list|조회|테스트|test|zip.{0,20}검사|설치\s*zip.{0,20}확인|checksum|체크섬|manifest|매니페스트|후보|계획|plan|보고서|report|문서\s*정리|범위\s*식별|BEAI-owned\s*범위|비아이.{0,12}범위)/i.test(text);
}
function hasBeaiOwnedScopeSignal(text) {
    return /(BEAI[-\s]?owned|비아이\s*소유|비아이레이어만|BEAI\s*Layer\s*only|beai-runtime|BEAI\s*Runtime|비아이\s*런타임|비아이\s*닥터|beai\s*doctor|비아이.{0,12}캐시|beai.{0,12}cache|quarantine|격리|백업|rollback|롤백)/i.test(text);
}
function hasActualMutationSignal(text) {
    return /(삭제|영구\s*제거|제거|지워|덮어쓰기|overwrite|write|수정|변경|바꿔|바꾸|change|교체|설치|install|적용|등록|생성|업데이트|update|upgrade|재시작|restart|발송|보내|게시|올려|결제|cron|크론|agent|에이전트|권한|permission|memory|메모리)/i.test(text);
}
function hasHighRiskExecutionSurfaceRequest(text) {
    const normalized = String(text || "").replace(/\s+/g, " ").trim();
    if (!hasActualMutationSignal(normalized))
        return false;
    if (hasExplicitRoutingNegationSignal(normalized))
        return false;
    if (/(어떻게|뭐야|무엇|설명|개념|가능|할\s*수|될지|판단|검토|후보|계획|초안|dry[-\s]?run|드라이런|확인만|조회만|상태만|목록만)/i.test(normalized)) {
        return false;
    }
    return /(해줘|해|바꿔|수정해|변경해|지워|삭제|설치|적용|등록|생성|올려|보내|발송|실행|눌러|submit|restart|재시작|켜줘|꺼줘|만들어)/i.test(normalized);
}
function isStrongApprovalRiskFamily(family) {
    return ["external_send", "delete", "payment", "browser_submit", "device_action", "permission_change", "durable_memory_write"].includes(family);
}
function inferApprovalActionSummary(text, risks) {
    if (risks.includes("external_send"))
        return "외부 채널로 메시지나 콘텐츠를 보내기";
    if (risks.includes("delete"))
        return "파일 또는 기존 설치 항목 삭제/제거";
    if (risks.includes("payment"))
        return "결제, 송금, 주문 같은 비용 발생 가능 작업";
    if (risks.includes("browser_submit"))
        return "브라우저 제출/확정 버튼 실행";
    if (risks.includes("cron_creation"))
        return "cron 또는 반복 자동 실행 등록";
    if (risks.includes("permission_change"))
        return "설정, 권한, 버전, OpenClaw 관련 변경";
    if (risks.includes("durable_memory_write"))
        return "장기 기억 또는 사용자 데이터 저장";
    if (risks.includes("install"))
        return "BEAI Layer 설치 또는 교체";
    if (risks.includes("restart"))
        return "서비스 또는 Gateway 재시작";
    if (hasLowRiskPreparationSignal(text))
        return "읽기/점검/dry-run/후보안 작성";
    return "요청한 작업 범위 처리";
}
function inferApprovalImpactScope(text, risks, level) {
    if (level === "auto_allowed")
        return "읽기 전용 점검, dry-run, 후보안 또는 보고서 범위";
    if (hasBeaiOwnedScopeSignal(text))
        return "BEAI Layer 소유 파일 또는 BEAI Runtime 설치 범위";
    if (risks.includes("external_send"))
        return "외부 채널, 고객/팀원 메시지, 공개 게시 범위";
    if (risks.includes("permission_change"))
        return "OpenClaw 설정, 권한, 버전, 배포 또는 런타임 설정 범위";
    if (risks.includes("durable_memory_write"))
        return "사용자 memory, 장기 기억, 프로젝트 상태 파일 범위";
    if (risks.includes("cron_creation"))
        return "OpenClaw cron/automation 등록 상태";
    return "변경 대상 파일, 설정, 실행 상태";
}
function inferApprovalDoesNotTouch(text, risks, level) {
    if (level === "auto_allowed")
        return "파일 변경, 외부 발송, cron/agent 등록, 설정 변경";
...
function inferApprovalRecovery(text, level) {
    if (level === "auto_allowed")
        return "변경이 없으므로 복구 대상 없음";
    if (hasBeaiOwnedScopeSignal(text))
...
export function buildApprovalErgonomicsProfile(input, report) {
    ...
    let level = "auto_allowed";
    let riskTransition = sameScopeContinuation ? "same_scope_continuation" : lowRiskPreparation ? "low_risk_preparation" : "none";
    ...
    return {
        ...
        guidance: level === "approval_required"
            ? [
...
export function classifyOperatingTelegramConfidence(input) {
    const text = String(input.currentInput || "");
    const normalized = text.toLowerCase();
    ...
    if (input.executionResultState === "completed")
        return { state: "complete", evidenceLevel };
    if (input.executionResultState === "partial" || input.executionResultState === "unverified")
        return { state: "partial", evidenceLevel };
    if (input.executionResultState === "blocked")
        return { state: "blocked", evidenceLevel };
    if (input.executionResultState === "failed")
        return { state: "failed", evidenceLevel };
    const mode = classifyExecutionMode(text);
    if (mode === "execution" || mode === "implementation")
        return { state: "running", evidenceLevel };
    if (mode === "diagnosis" || mode === "verification" || mode === "planning")
        return { state: "thinking", evidenceLevel };
    return { state: "listening", evidenceLevel };
}
export function classifyOperatingClaim(input) {
    const state = input.executionResultState;
    let label = "unverified";
    if (state === "completed")
        label = input.hasVerificationEvidence ? "complete" : "unverified";
    else if (state === "partial" || state === "skipped" || state === "unverified")
        label = "partial";
...
export function buildOperatingJudgmentReport(input) {
    const delegation = classifyOperatingDelegationCandidate(input);
    const riskFamilies = classifyOperatingRiskFamilies(input);
    const stateClasses = classifyOperatingStateHygieneClasses(input);
    const readOnlyInspection = hasReadOnlyAutomationInspectionSignal(input.currentInput) ||
        hasReadOnlyTelegramStatusSignal(input.currentInput) ||
        hasReadOnlyNotionSignal(input.currentInput);
    const strongApprovalRequired = !readOnlyInspection && riskFamilies.some((family) => ["external_send", "delete", "payment", "browser_submit", "device_action", "permission_change", "durable_memory_write"].includes(family));
...
export function buildSkillRoutingReport(input) {
    const rawText = input.currentInput || "";
    const text = normalizeSkillRoutingText(rawText) || rawText;
    const candidates = [];
    const suppressed = [];
    const add = (id, kind, title, score, triggerSignals, reasons, safetyNotes = []) => {
    ...
    if (/(배포|공유|zip|패키지|package|release|검증|버전|version|doctor|hooks|gateway|telegram|텔레그램)/i.test(text)) {
        add("beai-release-verifier", "skill", "BEAI release/package verification", 82, ["distribution_or_runtime_verification"], ["input asks about package, release, runtime, or operational verification"], ["verification should report checked and unchecked surfaces separately"]);
    }
    if (/(memory|메모리|기억|맥락|context|relevance|관련도|승격|후보)/i.test(text)) {
        add("beai-memory-curator-review", "skill", "BEAI memory candidate review", 76, ["memory_context_signal"], ["input concerns memory, context, relevance, or promotion boundaries"], ["do not promote memory automatically"]);
    }
    if (/(AI\s*네이티브|AX|OpenClaw|오픈클로|개인\s*(?:AI\s*)?(?:비서|assistant)|스킬|에이전트|크론|메모리|자동화).*?(헷갈|막막|어렵|뭘\s*먼저|어디서\s*시작|가이드|길|로드맵)|(?:헷갈|막막|어렵|뭘\s*먼저|어디서\s*시작).*?(AI\s*네이티브|AX|OpenClaw|오픈클로|스킬|에이전트|크론|메모리|자동화)/i.test(text)) {
        add("ai-native-journey-guide", "skill", "AI-native/OpenClaw journey guide", 84, ["ai_native_orientation_signal"], ["input asks for a friendly path through OpenClaw, AX, skills, agents, cron, memory, or personal AI assistant adoption"], ["guide the next safe success experience before implementation or automation"]);
    }
    if (/(cron|크론|스케줄|예약|자동\s*실행|매일|매주|모니터링|정기적|반복\s*실행|자동으로\s*(?:돌|보내|해)|(?:AI|OpenClaw|오픈클로).{0,24}(?:어떻게\s*풀|해결|맡기|줄이|운영)|(?:어떻게\s*풀|해결|맡기|줄이|운영).{0,24}(?:AI|OpenClaw|오픈클로))/i.test(text)) {
        add("automation-readiness-check", "skill", "Automation readiness and approval boundary check", 83, ["automation_readiness_signal"], ["input asks or implies scheduled, repeated, monitored, or unattended work"], ["manual proof, failure boundary, stop path, and approval gate must be checked before cron"]);
    }
    if (/(direct|skill|스킬|agent|에이전트|cron|크론|자동화|human-only|어디까지|분류|나눠|맡길|후보)/i.test(text) && !isMetaOnlySkillSignal(rawText)) {
        add("skill-agent-cron-router", "skill", "Direct / skill / agent / cron routing boundary", 72, ["capability_routing_signal"], ["input asks how work should be handled across direct answer, skill, agent, cron, automation, or human-only boundaries"], ["routing judgment only; do not create skills, agents, or cron directly"]);
    }
    if (/(직원|알바|매장|점장|대표.*(?:투입|갈려|들어가)|결근|근무표|인력|운영\s*병목|공정|마감|체크리스트)/i.test(text)) {
        add("operations-owner-load-diagnosis", "skill", "Owner load and operations bottleneck diagnosis", 84, ["owner_operations_load_signal"], ["input concerns owner overload, staff gaps, store operations, checklists, schedules, or recurring execution bottlenecks"], ["prefer observation/report-first workflow before staff messaging or schedule changes"]);
    }
    if (/(전환|구매전환|전환율|유입|상세페이지|장바구니|이탈|재구매|리뷰|고객\s*여정|광고|인스타|랜딩|첫\s*구매)/i.test(text)) {
        add("customer-journey-auditor", "skill", "Customer journey and conversion friction audit", 84, ["customer_journey_signal"], ["input concerns traffic, conversion, detail page, repeat purchase, review, ad-to-page mismatch, or customer journey friction"], ["separate traffic, trust, offer, detail-page, and action friction before recommending tactics"]);
    }
    if (/(매출|현금|고정비|마진|이익|수익|손익|비용|객단가|단가|가격|돈이\s*안\s*남|자금|회수)/i.test(text)) {
        add("eco-cash-pressure-diagnosis", "skill", "Cash pressure and business economics diagnosis", 74, ["money_or_cash_pressure_signal"], ["input concerns money, cash, margin, cost, price, payback, or business economics"], ["do not turn this into financial advice beyond visible operating evidence"]);
    }
    if (/(관점|상대|고객이\s*(?:보|느끼|선택)|선택\s*조건|상대적\s*관점|수신자|받아들|설득|메시지|문구|카피)/i.test(text)) {
        add("relative-perspective-lens", "skill", "Relative perspective and selection-condition lens", 70, ["relative_perspective_signal"], ["input asks to reframe from the other party's selection conditions or message reception"], ["translate claims into the receiver's usable selection language"]);
    }
    if ((hasExplicitSkillCreationSignal(text) || hasWorkflowCandidateSignal(text)) && !isMetaOnlySkillSignal(rawText)) {
        add("dagu-skill-foundry", "skill", "Skill candidate proposal workflow", hasExplicitSkillCreationSignal(text) ? 78 : 60, ["skill_or_workflow_signal"], ["input asks to connect repeated work to a skill or workflow candidate"], ["candidate proposal only; live skill application requires explicit approval"]);
    }
    if (/(handoff|세션|이어|다음\s*세션|연속성|continuity|압축|compaction)/i.test(text)) {
        add("beai-session-handoff", "skill", "Session continuity handoff", 74, ["session_continuity_signal"], ["input asks to carry work across sessions or preserve continuity"], ["carry only compact state, not broad transcript history"]);
    }
    if (/(agent|에이전트|독립|백그라운드|따로\s*일|위임|delegate)/i.test(text)) {
        add("agent-candidate-review", "agent", "Agent delegation candidate review", 56, ["agent_delegation_signal"], ["input mentions independent agent-style work"], ["do not spawn or route to an agent without explicit user approval and a clear stop condition"]);
    }
    for (const signal of input.roleSignals.slice(0, 4)) {
        const score = signal.priority === "high" ? 68 : 52;
        add(`role-${signal.role.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`, "manual_workflow", `${signal.role} workflow candidate`, score, [`role_signal:${signal.role}`], [signal.reason], ["role signal is evidence, not an automatic routing instruction"]);
    }
...
function inferWorkflowTitle(text) {
    if (hasRetryPolicySignal(text))
        return "실패 재시도 정책 후보";
    if (hasDestructiveRewriteSignal(text))
        return "파괴적 재작성 승인 후보";
    if (hasOpenClawCoreMutationSignal(text))
        return "OpenClaw core 변경 승인 후보";
    if (hasDurableProcedureSignal(text))
        return "반복 절차화 후보";
    if (/(proposal|제안|스킬).{0,24}(?:승인|적용|폐기|반려|격리|quarantine|apply|reject)/i.test(text))
        return "스킬 생애주기 변경 후보";
    if (/(마감|일마감|정산|pos|카드|계좌|입금|매출|지출)/i.test(text))
        return "일마감 정산 확인";
    if (/(예산|고정비|현금\s*흐름|비용|손익|미수금)/i.test(text))
        return "재무 운영 확인";
    if (/(콘텐츠|발행|업로드|게시|캠페인)/i.test(text))
        return "콘텐츠 발행 흐름 확인";
    if (/(고객|문의|응대|리뷰|컴플레인|불만)/i.test(text))
        return "고객 응대 흐름 확인";
    if (/(배포|검증|release|package|zip|doctor|hooks)/i.test(text))
        return "배포 검증 흐름 확인";
    if (hasScheduledWorkSignal(text))
        return "예약 자동화 후보";
    if (hasAppProductSignal(text))
    ...
    if (hasAgentCandidateSignal(text))
        return "에이전트 역할 후보";
    if (hasIssueLedgerWriteSignal(text))
        return "비아이 닥터 이슈 장부 기록 후보";
    return "반복 업무 후보";
}
function inferWorkflowOwner(text, skillRouting) {
    if (/(예산|고정비|현금\s*흐름|비용|손익|미수금|정산|매출|지출|입금)/i.test(text))
        return "ECO Finance Ops";
    if (/(콘텐츠|발행|업로드|게시|캠페인)/i.test(text))
        return "HERA Content Ops";
    if (/(매장|운영|마감|체크리스트|공정)/i.test(text))
...
function inferRequiredWorkflowInputs(text) {
    const inputs = [];
    const push = (value) => {
        if (!inputs.includes(value))
            inputs.push(value);
    };
    if (/(pos|포스|매출)/i.test(text))
        push("POS 매출");
    if (/(카드|정산)/i.test(text))
        push("카드 정산");
    if (/(계좌|입금|미수)/i.test(text))
        push("계좌 입금");
    if (/(지출|비용|고정비)/i.test(text))
        push("비용 지출");
    if (/(세금|부가세|통장)/i.test(text))
        push("세금/적립 계정");
    if (/(고객|문의|리뷰|컴플레인|불만)/i.test(text))
        push("고객 반응 기록");
    if (/(콘텐츠|발행|업로드|게시)/i.test(text))
        push("발행 대상 콘텐츠");
    if (/(배포|검증|package|zip|doctor|hooks)/i.test(text))
        push("패키지/검증 로그");
    return inputs;
}
function inferWorkflowRiskLevel(text, operatingJudgment) {
    if (operatingJudgment.risk.strongApprovalRequired)
        return "high";
...
function workflowSignalLevel(text, operatingJudgment) {
    if (hasReadOnlyAutomationInspectionSignal(text) || hasReadOnlyTelegramStatusSignal(text) || hasReadOnlyNotionSignal(text) || hasDocsOnlyReleaseArtifactSignal(text) || hasExplicitRoutingNegationSignal(text) || hasSkillDescriptionArtifactSignal(text))
        return "none";
    if (hasScheduledWorkSignal(text))
        return "scheduled_candidate";
    if (operatingJudgment.delegation.level === "L5_repeated_automation_candidate")
        return "scheduled_candidate";
    if (/(매일|매주|매월|정기|반복\s*실행|자동\s*실행|예약\s*(?:실행|발송|등록|작업))/i.test(text))
        return "scheduled_candidate";
    if (hasRetryPolicySignal(text))
        return "scheduled_candidate";
    if (hasDurableProcedureSignal(text))
    ...
    if (hasIssueLedgerWriteSignal(text))
        return "weak";
    if (hasSkillLifecycleSignal(text) || hasAgentCandidateSignal(text))
        return "weak";
    const releaseStatusOnly = /(openclaw|오픈클로|BEAI|비아이|runtime|plugin|플러그인|doctor|hooks|plugins\s+list|enabled|로드|검증|확인)/i.test(text) &&
        !/(반복|매번|자꾸|계속|귀찮|절차화|워크플로|workflow|routine|스킬|skill|agent|에이전트|cron|크론|자동화|후보|매일|매주|매월|정기|예약\s*(?:실행|발송|등록|작업))/i.test(text);
    if (releaseStatusOnly)
        return "none";
    if (/(반복|매번|자꾸|계속|귀찮|놓친|체크|확인|절차화|워크플로|workflow|routine|every\s+(day|week|month))/i.test(text)) {
        return "candidate";
    }
    if (hasSkillLifecycleSignal(text) || hasAgentCandidateSignal(text) || hasIssueLedgerWriteSignal(text) || /(스킬|skill|agent|에이전트|cron|크론|자동화|후보)/i.test(text))
        return "weak";
    return "none";
}
function inferWorkflowStage(signal, text, operatingJudgment) {
    if (signal === "none")
        return "no_candidate";
    if (/(실제\s*자동화|automation registry|자동화\s*목록|이미\s*예약|active automation)/i.test(text))
        return "automated";
    if (signal === "scheduled_candidate" || operatingJudgment.delegation.level === "L5_repeated_automation_candidate")
        return "cron_candidate";
    if (hasAgentCandidateSignal(text))
        return "agent_candidate";
    if (hasDurableProcedureSignal(text) || hasSkillLifecycleSignal(text) || hasIssueLedgerWriteSignal(text) || /(skill|스킬|절차화|프로세스화|workflow|워크플로)/i.test(text))
        return "skill_candidate";
    return "manual_test";
}
function buildPromotionConditions(stage) {
    const base = [
        "수동 실행 3회 성공",
        "입력 데이터 경로 고정",
        "결과 포맷 안정화",
...
function buildDoNotPromoteIf(riskLevel) {
    const blockers = [
...
function userFacingWorkflowStageSummary(stage, status) {
...
function buildWorkflowCard(input) {
    if (input.signal === "none" || input.stage === "no_candidate")
        return null;
    ...
    const status = input.stage === "automated"
        ? "automation_active"
...
function buildManualRunEvidence(card) {
    if (!card)
        return null;
    return {
...
function buildPromotionGateResult(card, manualRunEvidence, generatedAt) {
    if (!card || !manualRunEvidence)
        return null;
    const blockers = [
        "수동 실행 3회 성공 기록 없음",
        "입력 데이터 경로 안정성 미확인",
    ...
    if (card.externalSend)
        blockers.push("외부 발송은 기본 자동화 금지 범위입니다.");
    if (card.fileWrite)
        blockers.push("파일/장부 수정은 별도 승인 전 자동화하지 않습니다.");
    if (card.moneyMovement)
        blockers.push("돈 이동은 cron 승격 금지 범위입니다.");
    if (card.riskLevel === "high")
        blockers.push("high risk workflow는 strong approval과 별도 검증이 필요합니다.");
    const targetStage = card.stage === "skill_candidate" ? "skill" : card.stage === "agent_candidate" ? "agent" : card.stage === "automated" ? "automation_registry" : "cron";
    return {
        workflowId: card.workflowId,
        ...
        userFacingSummary: targetStage === "cron"
...
export function buildTurnPlan(prompt, options) {
    ...
    const sharedPlan = sharedCore.buildTurnPlan({
        ...
        ...inferredSignals,
        sessionContextUsagePct: options?.liveSessionContextUsagePct !== undefined && options?.liveSessionContextUsagePct !== null
            ? options.liveSessionContextUsagePct
            : inferredSignals.sessionContextUsagePct
    });
    const effectivePrimaryClass = hasReadOnlyAutomationInspectionSignal(planningPrompt) || hasReadOnlyTelegramStatusSignal(planningPrompt) || hasReadOnlyNotionSignal(planningPrompt)
        ? "verification"
        : sharedPlan.primaryClass;
    const constraints = sharedPlan.workOrder?.forbiddenActions?.map((item) => item.replace(/-/g, " ").trim()) ||
...
function sanitizeRepeatedFooterInstruction(value) {
    if (!value)
        return "";
    return value
...
export function renderPromptContext(plan, companionProfile) {
    ...
    if (plan.currentTurn.requestedOutputShape || plan.currentTurn.followUpScope !== "full" || plan.currentTurn.currentTarget) {
        ...
        if (plan.currentTurn.requestedOutputShape)
            lines.push(`- requested_output_shape: ${plan.currentTurn.requestedOutputShape}`);
        if (plan.currentTurn.followUpScope)
            lines.push(`- follow_up_scope: ${plan.currentTurn.followUpScope}`);
        if (plan.currentTurn.currentTarget)
            lines.push(`- current_target: ${plan.currentTurn.currentTarget}`);
    }
    ...
    if (plan.judgmentFrame.confirmed.length > 0) {
        ...
        for (const item of plan.judgmentFrame.confirmed.slice(0, 6))
            lines.push(`  - ${item}`);
    }
    if (plan.judgmentFrame.unknown.length > 0) {
        lines.push("- unknown:");
        for (const item of plan.judgmentFrame.unknown.slice(0, 4))
            lines.push(`  - ${item}`);
    }
    if (plan.judgmentFrame.assumptionCandidates.length > 0) {
        lines.push("- assumption_candidates:");
        for (const item of plan.judgmentFrame.assumptionCandidates.slice(0, 4))
            lines.push(`  - ${item}`);
    }
    if (plan.judgmentFrame.askOneQuestion)
        lines.push(`- ask_one_question: ${plan.judgmentFrame.askOneQuestion}`);
    if (plan.judgmentFrame.deferCondition)
        lines.push(`- defer_condition: ${plan.judgmentFrame.deferCondition}`);
    ...
    if (plan.handoffState) {
        ...
        if (plan.handoffState.current_track)
            lines.push(`- current_track: ${sanitizeContinuityStateText(plan.handoffState.current_track)}`);
        ...
        if (mustCarry.length > 0)
            lines.push(`- must_compare_before_answer: ${mustCarry.join(" | ")}`);
        if (discard.length > 0)
            lines.push(`- must_not_carry: ${discard.join(" | ")}`);
    }
    ...
    if (plan.sessionPressure?.level && plan.sessionPressure.level !== "ok") {
        ...
        for (const reason of plan.sessionPressure.reasons || [])
...
export function renderSessionSplitApprovalReply(plan) {
    ...
    if (starter) {
        lines.push("");
        if (starter.currentState)
            lines.push(`제가 먼저 지금까지 정한 기준을 짧게 정리하겠습니다. ${starter.currentState}`);
        if (starter.nextSessionGoal)
            lines.push(`그다음 바로 이어갈 수 있게 다음 작업 기준으로 묶겠습니다. ${starter.nextSessionGoal}`);
...
export function buildSessionContinuityState(plan) {
    const handoffState = plan.handoffState;
...
function inferConversationOrigin(plan) {
    const input = plan.currentTurn.cleanInput;
...
function inferConversationTurningPoints(plan, continuity) {
    ...
    const derived = [];
    const input = plan.currentTurn.cleanInput;
    if (/검증|근거|현실|정합|허상|거짓/i.test(input))
        derived.push("답변의 주장 강도는 확인된 근거를 넘지 않아야 한다는 기준이 중요해졌습니다.");
    if (/세션|context|컨텍스트|압축|새\s*세션|흐름/i.test(input))
        derived.push("세션 전환은 정보 손실뿐 아니라 함께 온 흐름의 단절 문제로 다뤄야 한다는 기준이 생겼습니다.");
    if (/메모리|memory|기억|합의/i.test(input))
        derived.push("새 세션은 handoff만이 아니라 선별된 기억과 합의 기준을 함께 써야 한다는 방향이 확인됐습니다.");
    return Array.from(new Set([...derived, ...points])).slice(0, 6);
}
function inferDiscardedPaths(plan, continuity) {
...
function inferUserConcerns(plan) {
    const input = plan.currentTurn.cleanInput;
    const concerns = [];
    if (/허상|거짓|근거|정합|현실|신뢰/i.test(input))
        concerns.push("사용자는 AI 답변이 근거와 현실성을 잃지 않는 것을 중요하게 보고 있습니다.");
    if (/세션|context|컨텍스트|압축|새\s*세션|끊|단절|답답/i.test(input))
        concerns.push("사용자는 세션 전환이 대화 흐름의 단절로 느껴지는 문제를 중요하게 보고 있습니다.");
    if (/초심자|불편|불안|막막/i.test(input))
        concerns.push("사용자는 OpenClaw 초심자가 느끼는 불편과 불안을 줄이는 방향을 중요하게 보고 있습니다.");
    if (/메모리|memory|기억|합의/i.test(input))
        concerns.push("사용자는 기억을 많이 남기는 것보다 필요한 기준만 정확히 이어가는 것을 중요하게 보고 있습니다.");
...
export function buildContinuityJudgmentProfile(plan, continuity = buildSessionContinuityState(plan), arc = buildConversationArcCapsule(plan, continuity)) {
    const input = plan.currentTurn.cleanInput;
    ...
    const continuityNeed = splitRecommended
    ...
    const memoryBoundary = /기억|memory|장기|합의|agreement/i.test(input)
        ? "memory_review_required"
        : continuityNeed === "handoff_ready" || continuityNeed === "session_split_recommended"
            ? "session_only"
            : "project_state_candidate";
    const openingStyle = continuityNeed === "none" ? "none" : continuityNeed === "light" ? "short_resume" : "next_action_first";
    const answerGuidance = [
...
function detectJudgmentFlowDomain(text) {
    if (/(김제|집터|아버지|증조|토지|농막|건축|부동산|상시\s*거주|체류)/i.test(text))
        return "family_asset";
    if (hasBusinessOperationsSignal(text))
        return "business_operations";
    if (/(BEAI|비아이|OpenClaw|오픈클로|릴리즈|배포|버전|plugin|플러그인|runtime|gateway|telegram|hooks|doctor|rollback|zip)/i.test(text))
...
function evidenceMatchesCheck(check, evidenceText, domain) {
    const normalizedCheck = check.toLowerCase();
    const normalizedEvidence = evidenceText.toLowerCase();
    if (!normalizedCheck || !normalizedEvidence)
        return false;
    ...
    if (domain === "business_operations") {
        if (/순이익|profit|손익/i.test(check))
            return /(순이익|영업이익|손익|profit|margin|마진)/i.test(evidenceText);
        if (/긴급투입|투입|시간/i.test(check))
            return /(긴급투입|투입|시간|몸빵|shift|hours)/i.test(evidenceText);
        if (/2번 선수|대체|공정|백업/i.test(check))
            return /(2번|백업|대체|공정|매뉴얼|SOP|backup)/i.test(evidenceText);
    }
    if (domain === "family_asset") {
        if (/지목|접도|상하수도|정화조|건축/i.test(check))
            return /(지목|접도|도로|상하수도|정화조|건축|인허가|토지이음)/i.test(evidenceText);
        if (/상시 거주|간헐 체류/i.test(check))
            return /(상시\s*거주|간헐\s*체류|주말|체류|거주)/i.test(evidenceText);
    }
...
function buildEvidenceClosureReport(domain, unresolvedChecks, ledger) {
    ...
    const state = closedChecks.length === 0
        ? "open"
...
export function buildJudgmentFlowProfile(plan) {
    const input = plan.currentTurn.cleanInput;
    ...
    if (domain === "business_operations") {
        ...
        if (/(법인|종소세|법인세|세금|1억|자금)/i.test(input))
            changedVariables.push("법인전환과 2027년 세금/현금 방어 변수");
        if (/(직원|인력|코어|몸빵|주방|빠지)/i.test(input))
            changedVariables.push("코어 인력 이탈 시 오너가 현장에 빨려 들어가는 운영 변수");
        if (/(가족|아들|와이프|아내|시간)/i.test(input))
            changedVariables.push("가족 시간과 부부 역할 소모 변수");
        ...
        if (/(대출|6천|3천|세금|공시지가|국가\s*소유|특조)/i.test(input))
            changedVariables.push("취득 비용, 대출, 특조/토지 권리 변수");
        if (/(아버지|살고|농막|완세트)/i.test(input))
            changedVariables.push("아버지의 거주 희망과 실제 생활 인프라 변수");
        ...
        if (/(버전|v0\.|0\.4|릴리즈|배포|zip)/i.test(input))
            changedVariables.push("버전/패키지 라벨 정합성 변수");
        if (/(gateway|telegram|hooks|doctor|plugin|runtime|승인|approval)/i.test(input))
            changedVariables.push("OpenClaw 런타임 상태와 hook/approval 경계 변수");
    ...
    for (const item of plan.constraints)
        stableCriteria.push(item);
    for (const item of plan.realitySignalProfile.needsVerification)
        unresolvedChecks.push(item);
    ...
    const posture = hasUnresolved
        ...
        : hasChangedVariables
            ...
            : plan.continuityJudgment.continuityNeed !== "none"
                ...
                : "choose_next_action";
    const nextJudgmentHandle = domain === "business_operations"
...
export function buildNewSessionContextPack(plan) {
...
export function renderWorkOrderReply(plan) {
    if (!plan.workOrder)
    ...
    for (const [index, step] of plan.workOrder.steps.entries())
        lines.push(`${index + 1}. ${step}`);
    if (plan.workOrder.constraints.length > 0) {
        lines.push("", "제약 조건:");
        for (const item of plan.workOrder.constraints)
            lines.push(`- ${item}`);
    }
    lines.push("", "삭제, 외부 전송, 배포는 하지 않는 조건입니다.");
    lines.push("이 범위로 작업 지시를 만들면 됩니다.");
    if (plan.requiresUserConfirmation)
        lines.push("고위험 변경은 승인 전까지 진행하지 않습니다.");
    return lines.join("\n");
}
function isInstallGuideObjective(value) {
    const normalized = String(value || "").toLowerCase();
    if (!normalized.trim())
        return false;
    return /install|overlay|package|zip|telegram|messenger|setup|onboard|설치|패키지|오버레이|텔레그램|메신저|온보딩/.test(normalized);
}
export function renderInstallGuideReply(plan, installContext) {
    const lines = [
    ...
    const relevantSignals = plan.roleSignals.filter((signal) => ["Worker", "Verifier", "Facility"].includes(signal.role));
...
export function renderInstallResumeReply(installContext) {
    const lines = [
...
export function extractFinalResponse(text) {
...
function safeFallbackReply(plan) {
    if (!plan)
        return "핵심만 정리해서 다시 답하겠습니다.";
    if (isInternalRecoveryLeak(plan.currentTurn.cleanInput)) {
        return recoveryLeakFallbackReply();
    }
    if (plan.mode === "handoff")
        return renderWorkOrderReply(plan);
    if (plan.mode === "planning")
        return "실행 전에 범위와 우선순위를 먼저 정리하겠습니다.";
    return "핵심만 바로 답하고 내부 디버그 내용은 제외하겠습니다.";
}
function recoveryLeakFallbackReply() {
    return "방금 OpenClaw 작업 흐름이 중간에 끊겼던 신호가 있습니다. 내부 복구 문장은 제외하고, 현재 연결 상태와 이어서 처리할 일을 확인하겠습니다.";
}
export function isInternalRecoveryLeak(text) {
    if (!text)
        return false;
    const normalized = text.toLowerCase().replace(/\s+/g, " ").trim();
    return (/\[system\]\s*your previous turn was interrupted/.test(normalized) ||
...
export function isInternalSessionDeliveryArtifact(text) {
    if (!text)
        return false;
    ...
    return (/^\[inter-session message\]/i.test(normalized) ||
...
export function isInternalProcessSurface(text) {
    if (!text)
        return false;
    const normalized = text.replace(/\r\n/g, "\n").trim();
    if (!normalized)
        return false;
    if (/^\[beai internal process surface isolated\]/i.test(normalized))
        return true;
    if (isInternalSessionDeliveryArtifact(normalized))
        return true;
    if (isInternalRecoveryLeak(normalized))
        return true;
    if (isCodexCommandApprovalSurface(normalized))
        return true;
    if (isApprovalWaitSurface(normalized))
        return true;
    ...
    return ((hasPinchingHeader && processLines.length >= 2) ||
...
export function renderInternalProcessSurfaceGuide(text) {
    if (isCodexCommandApprovalSurface(text))
        return renderCodexCommandApprovalGuide(text);
    if (isApprovalWaitSurface(text))
        return renderApprovalWaitGuide(text);
    if (isInternalRecoveryLeak(text))
        return recoveryLeakFallbackReply();
    return [
...
function stripDebugTail(text) {
    return text
        .replace(/\n(?:debugsummary|디버그\s*요약|debug\s*summary)\s*:[\s\S]*$/i, "")
        .replace(/\n(?:internal\s*notes|internal\s*debug)\s*:[\s\S]*$/i, "")
        .trim();
}
function stripLeadingOverlayBlock(text) {
    if (!text.trimStart().toLowerCase().startsWith("[beai runtime overlay]"))
        return text.trim();
...
function stripInternalSurfaceLabels(text) {
    let result = text;
...
function limitSurfaceQuestions(text) {
    let kept = false;
    const segments = text
        .replace(/([?？])/g, "$1\n")
        .split("\n");
...
function detectLastHandle(text) {
    const normalized = text.replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
...
function detectArtifactDelayedByExplanation(text, plan) {
    if (!plan || !["artifact", "plan", "work_order"].includes(plan.judgmentFrame.responseRole))
        return false;
...
function appendLastHandle(text, plan) {
    if (!plan)
        return text;
    if (detectLastHandle(text))
        return text;
    const deferCondition = plan.judgmentFrame.deferCondition;
    if (!deferCondition)
        return text.trim();
    if (isInternalRecoveryLeak(deferCondition))
        return text.trim();
    return `${text.trim()}\n\n보류 조건은 ${deferCondition}`;
}
export function checkSurfaceLanguage(text, plan) {
    ...
    if (internalLabelsFound.length > 0)
        suggestions.push("내부 구조 라벨을 사용자 언어로 바꿉니다.");
    if (questionCount > 1)
        suggestions.push("질문을 하나 이하로 줄입니다.");
    if (!hasLastHandle)
        suggestions.push("마지막에 기준, 행동, 확인 신호, 보류 조건 중 하나를 남깁니다.");
    if (artifactDelayedByExplanation)
        suggestions.push("산출물 요청은 설명보다 결과물을 먼저 배치합니다.");
    return {
        internalLabelsFound,
        questionCount,
        hasLastHandle,
...
export function applySurfaceLanguageGuard(text, plan) {
    ...
    const grounded = softenChoiceDominationLanguage(softenUnverifiedActionAndHighStakesClaims(softenInterpersonalSycophancyLanguage(softenOverclaimingLanguage(guarded, plan)), plan), plan);
    return appendLastHandle(grounded, plan);
}
function guardAmbiguousPermissionOverapproval(text, plan) {
    if (!plan || !isAmbiguousPermissionContinuationRequest(plan.currentTurn.cleanInput))
        return text;
    const normalized = text.replace(/\s+/g, " ").trim();
    const overApproval = /^(네|예|응|맞습니다)[,.\s]*(?:윤[,.]?\s*)?.{0,32}(만들어도\s*됩니다|진행해도\s*됩니다|해도\s*됩니다|만들\s*수\s*있는\s*상태|진행\s*가능)/i.test(normalized);
    if (!overApproval)
        return text;
    const candidate = plan.handoffState?.next_action ||
        plan.handoffState?.current_track ||
        plan.continuityPatch.current_focus ||
        "이전 맥락에서 회수된 후보";
...
export function sanitizeUserFacingReply(text, plan) {
    const trimmed = text.trim();
    if (!trimmed)
        return null;
    ...
    if (finalOnly)
        return applySurfaceLanguageGuard(finalOnly, plan);
    const stripped = stripDebugTail(stripLeadingOverlayBlock(trimmed));
    if (stripped && stripped !== trimmed)
        return applySurfaceLanguageGuard(stripped, plan);
...
export function isCodexCommandApprovalSurface(text) {
    if (!text)
        return false;
    const normalized = text.toLowerCase().replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
    const hasCodexApproval = normalized.includes("codex app-server command approval") ||
...
export function isApprovalWaitSurface(text) {
    if (!text)
        return false;
    const normalized = text.toLowerCase().replace(/\s+/g, " ").trim();
    if (!normalized)
...
export function classifyTelegramUxState(text) {
    if (!text)
        return null;
    const normalized = text.toLowerCase().replace(/\s+/g, " ").trim();
    if (!normalized)
...
export function renderApprovalWaitGuide(text) {
...
export function renderCodexCommandApprovalGuide(text) {
...
export function renderTelegramUxStateGuide(state) {
    if (state.kind === "codex_command_approval_wait")
        return renderCodexCommandApprovalGuide();
    if (state.kind === "approval_wait")
        return renderApprovalWaitGuide();
    return [
        "현재 상태 안내입니다.",
        "",
        state.userMeaning,
...
function isLikelyMetaArtifact(value) {
    if (!value)
        return false;
...
function compactList(values, itemLimit, textLimit) {
...
function mapExecutionHeader(label) {
    const normalized = normalizeHeaderLabel(label);
    if (normalized === "what was checked" || normalized === "무엇을 확인했는지")
        return "whatChecked";
    if (normalized === "what was changed" || normalized === "무엇을 적용했는지" || normalized === "무엇을 바꿨는지")
        return "whatChanged";
    if (normalized === "what was not changed" || normalized === "무엇을 바꾸지 않았는지")
        return "whatNotChanged";
    if (normalized === "result status" || normalized === "결과 상태")
        return "resultStatus";
    if (normalized === "remaining risks or unverified items" ||
        normalized === "남아 있는 미확인 또는 리스크" ||
        normalized === "남은 리스크 또는 미검증 항목") {
        return "remainingRisks";
    }
    if (normalized === "recommended next step" || normalized === "권장 다음 단계" || normalized === "다음 조치")
        return "recommendedNextStep";
    return null;
}
const EXECUTION_NOTE_EXACT_MAP = {
    "local build": "로컬 빌드",
...
function localizeExecutionItem(value, kind) {
    const trimmed = value.trim();
    if (!trimmed)
        return "";
    ...
    if (exact)
        return exact;
    if (kind === "risk") {
        if (/live telegram roundtrip/i.test(trimmed))
            return "실제 Telegram 왕복 응답";
        if (/live surface smoke check/i.test(trimmed))
            return "실제 표면 점검";
    }
    if (kind === "nextStep") {
...
function localizeExecutionDraft(draft) {
    return {
        ...draft,
...
function joinNaturalItems(items) {
...
function allSentenceLike(items) {
    return items.length > 0 && items.every((item) => isSentenceLikeText(item));
}
function renderExecutionReviewFallback(draft) {
    const lines = [];
...
function mapExecutionResultState(draft) {
...
function defaultNextCheckForState(state, draft) {
    if (draft.recommendedNextStep?.trim())
        return draft.recommendedNextStep.trim();
    if (draft.remainingRisks?.[0])
        return `${draft.remainingRisks[0]}을 먼저 확인합니다.`;
    if (state === "completed")
        return "필요하면 실제 표면에서 한 번 더 확인합니다.";
    if (state === "failed")
        return "실패 지점을 하나로 좁혀 다시 확인합니다.";
    if (state === "blocked")
        return "막힌 조건이 풀리는지 먼저 확인합니다.";
    if (state === "skipped")
        return "건너뛴 이유가 여전히 유효한지 확인합니다.";
    return "실제 결과를 한 번 더 확인합니다.";
}
export function buildExecutionReview(text) {
...
export function classifyInstallResultLabel(text, review) {
    ...
    if (review?.resultState === "failed")
        return "blocked";
    return null;
}
export function classifyTelegramEvidenceLevelFromText(text) {
    const normalized = String(text || "");
    const roundtripMissing = /(roundtrip.*not checked|live telegram roundtrip not checked|왕복.*미확인|왕복.*확인.*못|telegram.*왕복.*확인.*못|텔레그램.*왕복.*미확인)/i.test(normalized);
    if (roundtripMissing) {
        const hasGatewayChannelWhileMissing = /(telegram.*(on\s*\/\s*ok|\bok\b|connected|정상)|텔레그램.*(on\s*\/\s*ok|\bok\b|연결|정상)|gateway.*telegram.*(\bok\b|reachable|connected)|channel.*telegram.*(\bok\b|reachable|connected))/i.test(normalized);
        return hasGatewayChannelWhileMissing ? "gateway_channel_reachable" : "configured";
    }
    const hasRoundtrip = /(roundtrip.*(checked|verified|ok|success)|왕복.*(확인|성공|정상)|실제.*telegram.*왕복.*확인|telegram.*실제.*응답.*확인|live\s*telegram\s*roundtrip\s*verified)/i.test(normalized);
    if (hasRoundtrip)
        return "live_roundtrip_verified";
    const hasGatewayChannel = /(telegram.*(on\s*\/\s*ok|\bok\b|connected|정상)|텔레그램.*(on\s*\/\s*ok|\bok\b|연결|정상)|gateway.*telegram.*(\bok\b|reachable|connected)|channel.*telegram.*(\bok\b|reachable|connected))/i.test(normalized);
    if (hasGatewayChannel)
        return "gateway_channel_reachable";
    const hasConfigured = /(telegram.*(configured|enabled|token config)|텔레그램.*(설정|활성|토큰))/i.test(normalized);
    if (hasConfigured)
        return "configured";
    return "not_applicable";
}
function installResultLabelText(label) {
    switch (label) {
        case "complete":
...
function telegramEvidenceSurfaceText(level) {
    switch (level) {
        case "configured":
...
function renderStructuredExecutionReview(review) {
    const lines = [];
    ...
    if (review.changed.length > 0)
        lines.push(`${joinNaturalItems(review.changed)}은 반영됐습니다.`);
    if (review.verified.length > 0)
        lines.push(`${joinNaturalItems(review.verified)} 범위는 확인했습니다.`);
    if (review.notChanged.length > 0)
        lines.push(`${joinNaturalItems(review.notChanged)}은 바꾸지 않았습니다.`);
    if (review.unverified.length > 0)
        lines.push(`다만 ${joinNaturalItems(review.unverified)}은 아직 확인하지 못했습니다.`);
    lines.push(review.nextCheck.startsWith("다음") ? review.nextCheck : `다음으로는 ${review.nextCheck}`);
    return lines.join("\n");
}
function parseExecutionReviewDraft(text) {
...
export function normalizeExecutionReviewReply(text, plan) {
    const draft = parseExecutionReviewDraft(text);
    if (!draft)
        return null;
    const localizedDraft = localizeExecutionDraft(draft);
...
export function compactWorkingMemoryPatch(patch) {
...
function inferProjectNextStep(plan) {
    const input = plan.currentTurn.cleanInput;
    if (plan.handoffState?.next_action)
        return plan.handoffState.next_action;
    if (plan.workOrder?.steps?.[0])
        return plan.workOrder.steps[0];
    if (/다음\s*세션|새\s*세션|세션\s*연속성|이어갈\s*수\s*있게|handoff|continuity/i.test(input)) {
        return "다음 세션이 이어받을 세션 연속성 기준과 다음 행동만 짧게 남깁니다.";
    }
    if (plan.continuityPatch.current_focus)
        return plan.continuityPatch.current_focus;
    if (plan.objective)
        return plan.objective;
    if (plan.acceptanceChecks?.[0])
        return plan.acceptanceChecks[0];
    return "다음 작업 기준을 확인합니다.";
}
function inferVerificationStatus(plan) {
    if (plan.requiresUserConfirmation) {
...
function compactBullets(values, limit) {
    if (!Array.isArray(values))
        return [];
    return values
...
function renderProjectStateSnapshot(plan, finalText) {
...
export function updateProjectStateSnapshot(workspaceDir, plan, finalText) {
    if (!workspaceDir)
        return;
...
function isDurableAgreementLine(value) {
...
function extractAgreementCandidates(plan, finalText) {
    const candidates = new Map();
    const push = (statement, rationale) => {
        const compact = compactText(statement, 220);
        if (!compact || !isDurableAgreementLine(compact))
            return;
...
function mergeUniqueByText(existing, incoming, limit = 80) {
...
function hasDoNotRememberSignal(text) {
    return /(기억하지\s*마|기억하지\s*말|저장하지\s*마|남기지\s*마|forget\s+this|do\s+not\s+remember|don't\s+remember)/i.test(text);
}
function hasExplicitAgreementSignal(text) {
    return /(기억해|앞으로\s*이렇게|이\s*기준\s*유지|기준으로\s*유지|합의|원칙으로\s*하자|keep\s+this|remember\s+this|from\s+now\s+on)/i.test(text);
}
function isTransientMemoryLine(text) {
    const normalized = text.trim();
    if (!normalized)
        return true;
    if (normalized.length < 12)
        return true;
    if (isLikelyMetaArtifact(normalized))
        return true;
    if (INTERNAL_REPLY_TERMS.some((term) => normalized.toLowerCase().includes(term.replace(/\s+/g, ""))))
        return true;
    if (/(debugSummary|handoffState|diagnosticFamily|finalResponse|What was checked|Result status)/i.test(normalized))
        return true;
    if (/(좋아|고마워|알겠어|오케이|응|네)[.!?。！？]?$/.test(normalized) && normalized.length < 30)
        return true;
    return false;
}
function classifyMemoryScope(text) {
    if (/(말투|호칭|이름|존댓말|반말|가깝|거리|비서|동반자|tone|style)/i.test(text))
        return "personal_preference";
    if (/(승인|허락|금지|하지\s*마|절대|without approval|approval)/i.test(text))
        return "approval_boundary";
    if (/(철학|원칙|기준|목표|방향|구조|1차\s*대상|우선\s*대상|먼저\s*(?:닫|처리|구현)|first\s*(?:implementation\s*)?target|primary\s*(?:implementation\s*)?target|runtime\/package|OpenClaw\s*Runtime|OpenClaw\s*Package|BEAI\s*Runtime|BEAI\s*Package|Codex\s*Harness.*later|OpenClaw\s*코어|오픈클로\s*코어|배포\s*전|명시\s*지시)/i.test(text))
        return "project_principle";
    if (/(완료|진행|작업|테스트|빌드|status|상태|다음\s*단계|구현|수정)/i.test(text))
        return "execution_state";
    if (/(세션|흐름|맥락|context|handoff|압축|이어)/i.test(text))
        return "conversation_flow";
    return "unknown";
}
function hasLongTermStandardSignal(text) {
    return /(앞으로|항상|절대|반드시|계속|유지|원칙|기준|합의|승인\s*전|명시\s*지시|do\s*not|never|always|from\s*now\s*on|keep\s*this)/i.test(text);
}
function isPlainWorkflowUpdate(text) {
    if (hasExplicitAgreementSignal(text) || hasLongTermStandardSignal(text))
        return false;
...
function classifyMemoryTopic(text) {
    if (/(배포|공유|zip|package|install|설치|rollback|롤백|version|버전|v0\.)/i.test(text))
        return "distribution";
    if (/(memory|메모리|기억|context|맥락|relevance|관련도|candidate|후보)/i.test(text))
        return "memory";
    if (/(skill|스킬|반복|자동화|automation|agent|에이전트)/i.test(text))
        return "skill";
    if (/(승인|approval|고위험|위험|금지|허락|안전)/i.test(text))
        return "approval";
    if (/(gateway|telegram|텔레그램|hook|훅|runtime|런타임|openclaw|오픈클로|beai|비아이)/i.test(text))
        return "runtime";
    return "general";
}
function classifyMemoryStaleness(createdAt, now = new Date()) {
    if (!createdAt)
        return "unknown";
...
export function evaluateMemoryRelevance(candidate, currentInput, now = new Date()) {
    ...
    const relation = score >= 70 ? "direct" : score >= 50 ? "adjacent" : score >= 30 ? "background" : "unrelated";
    const action = score >= 60 && stalenessRisk !== "high"
        ? "inject_candidate"
        : score >= 40
            ? "review_only"
            : "defer";
...
export function classifyMemoryCurrentJudgmentImpact(relevance, policy) {
    if (relevance.action === "discard" || relevance.relation === "unrelated")
        return "none";
    if (policy?.decision === "discard" || policy?.consentLevel === "do_not_store")
        return "none";
    const highImpactScope = ["project_principle", "approval_boundary", "execution_state", "conversation_flow"];
    if (relevance.action === "inject_candidate" &&
        relevance.score >= 80 &&
        policy?.scope &&
        highImpactScope.includes(policy.scope)) {
        return "high";
    }
    if (relevance.action === "inject_candidate" && relevance.score >= 60)
        return "medium";
    if (relevance.action === "review_only" && relevance.score >= 40)
        return "low";
    return "none";
}
export function classifyFlowMemoryInfluenceType(policy) {
    if (!policy || policy.decision === "discard")
        return "discard";
    if (policy.decision === "route_session_continuity" || policy.scope === "conversation_flow")
        return "session_continuity";
    if (policy.decision === "route_project_state" ||
        ["project_principle", "approval_boundary", "execution_state"].includes(policy.scope)) {
        return "project_state";
    }
    if (policy.decision === "store_candidate" && policy.durability === "long_term")
        return "long_term_memory_candidate";
    if (policy.decision === "store_candidate")
        return "package_knowledge";
    return "discard";
}
export function evaluateMemoryCandidatePolicy(text, source = "current_turn") {
    const normalized = text.trim();
...
function buildBeaiMemoryCandidates(plan, finalText) {
    if (hasDoNotRememberSignal(plan.currentTurn.cleanInput))
        return [];
    const now = new Date().toISOString();
    const candidates = [];
    const push = (text, source, reason) => {
        const compact = compactText(text, 260);
        if (!compact || isTransientMemoryLine(compact))
            return;
    ...
    for (const item of plan.constraints)
        push(item, "runtime_constraint", "runtime constraint candidate");
    for (const item of plan.handoffState?.decisions_made || [])
        push(item, "handoff_state", "handoff decision candidate");
    if (finalText) {
        push(summarizeReply(finalText), "final_reply", "final reply memory policy accepted");
    }
    return mergeUniqueByText([], candidates, 20);
}
export function buildMemoryRelevanceReport(candidates, plan, finalText, now = new Date()) {
...
export function buildFlowMemoryInfluenceFromRelevanceReport(report) {
...
function buildBeaiAgreementAssets(plan, finalText) {
    if (!hasExplicitAgreementSignal(plan.currentTurn.cleanInput))
        return [];
    const now = new Date().toISOString();
    const assets = [];
    const push = (statement, reason) => {
...
function buildBeaiProjectStateAsset(plan, finalText) {
...
function inferDeploymentBoundary(plan, finalText) {
    const text = [
...
function inferRelevantArtifacts(plan, finalText) {
    const text = [
        plan.currentTurn.cleanInput,
        plan.objective,
        finalText,
    ...
    for (const match of text.matchAll(/\b[\w.-]+\.zip\b/g))
        artifacts.add(match[0]);
...
function buildDiscardedContextItems(plan, finalText) {
    const discarded = [];
    const now = new Date().toISOString();
    const push = (text, reason) => discarded.push({ text, reason, discardedAt: now });
    if (hasDoNotRememberSignal(plan.currentTurn.cleanInput))
        push(plan.currentTurn.cleanInput, "user explicitly asked not to remember this turn");
    if (finalText && isTransientMemoryLine(finalText))
        push(compactText(finalText, 260) || "transient final reply", "transient or internal-looking text");
    return discarded;
}
...
export function renderCompanionSetupPrompt() {
    return [
...
function detectRelationshipDistance(text) {
    if (/격식|비서|formal|정중/i.test(text))
        return "formal";
    if (/가까운|동반자|친근|close/i.test(text))
        return "close";
    if (/실무|도구|조력자|practical/i.test(text))
        return "practical";
    return "warm";
}
function detectMemoryPreference(text) {
    if (/최소|적게|거의\s*기억하지|minimal/i.test(text))
        return "minimal";
    if (/물어|허락|동의|ask/i.test(text))
        return "ask-first";
    return "candidate-first";
}
function detectAutomationBoundary(text) {
    if (/직접 말할 때만|수동|manual|내가 시킬 때/i.test(text))
        return "manual-first";
    if (/낮은 위험|루틴|반복|low-risk/i.test(text))
        return "allow-low-risk-routines";
    return "confirm-before-scheduling";
}
function extractPrimaryUseCases(text) {
    const cases = [];
    const checks = [
...
function extractTone(text) {
    const tones = [];
    if (/차분|calm/i.test(text))
        tones.push("차분하게");
    if (/따뜻|warm/i.test(text))
        tones.push("따뜻하게");
    if (/간결|짧|concise/i.test(text))
        tones.push("간결하게");
    if (/단호|명확|선명/i.test(text))
        tones.push("명확하게");
    if (/친근/i.test(text))
        tones.push("친근하게");
    return tones.length > 0 ? `${tones.join(", ")} 답합니다.` : defaultCompanionProfile().userFacingTone;
}
function extractNeverDoWithoutApproval(text) {
...
export function buildCompanionProfileFromText(text, existing) {
...
function companionProfilePath(workspaceDir) {
    return path.join(workspaceDir, "state", "beai", "companion-profile.json");
}
export function loadCompanionProfile(workspaceDir) {
    if (!workspaceDir)
        return undefined;
    try {
...
export function saveCompanionProfile(workspaceDir, profile) {
    if (!workspaceDir)
        return;
    writeJsonFile(companionProfilePath(workspaceDir), profile);
}
export function renderCompanionProfileContext(profile) {
    return [
        "companion_profile:",
...
export function shouldOfferCompanionSetup(input) {
    return /(첫\s*설정|처음\s*설정|동반자\s*설정|컴패니언|companion|내\s*ai|나만의\s*ai)/i.test(input);
}
export function shouldSaveCompanionProfile(input) {
    return /(이렇게\s*설정|이걸\s*기준|프로필로\s*저장|설정해줘|기억해줘|앞으로\s*이렇게)/i.test(input) && /(말투|기억|승인|자동|동반|비서|조력자|이름|호칭|용도)/i.test(input);
}
function detectCapabilityKind(input) {
    if (/자동화|반복|매일|매주|주기|시간|알림|깨워|cron|예약|스케줄/i.test(input))
        return "cron";
    if (/기억|메모리|remember|memory|계속\s*기억/i.test(input))
        return "memory";
    if (/플러그인|plugin|설치|연결|확장/i.test(input))
        return "plugin";
    if (/스킬|skill|잘하게|능력/i.test(input))
        return "skill";
    if (/에이전트|agent|독립|따로\s*일|백그라운드/i.test(input))
        return "agent";
    return "manual-workflow";
}
function inferCapabilityReadiness(input, kind) {
    if (/(바로\s*만들|설정해|등록해|매일|매주|오전|오후|\d+\s*시)/i.test(input)) {
        if (/(돈|결제|삭제|외부\s*전송|메일\s*보내|배포|위험|고위험)/i.test(input))
            return "recommended";
        return kind === "cron" ? "ready-to-configure" : "recommended";
    }
    if (/(어떻게|뭐야|무엇|궁금|알려|설명)/i.test(input))
        return "candidate";
    return kind === "manual-workflow" ? "recommended" : "candidate";
}
function capabilityExplanation(kind) {
    switch (kind) {
        case "cron":
...
function capabilityWhyNow(input, kind, readiness) {
    if (kind === "cron" && readiness === "ready-to-configure")
        return "시간, 반복 주기, 할 일이 비교적 분명해서 설정 후보로 볼 수 있습니다.";
    if (kind === "cron")
        return "반복 가능성은 보이지만 실패 범위와 승인 경계를 먼저 정하는 편이 안전합니다.";
    if (kind === "memory")
        return "다시 설명하지 않아도 되는 기준인지 확인할 필요가 있습니다.";
    if (kind === "plugin")
        return "기능을 넓히기 전에 기존 OpenClaw 흐름과 충돌하지 않는지 확인해야 합니다.";
    if (kind === "agent")
        return "메인 대화와 분리할 만큼 독립적인 작업인지 먼저 봐야 합니다.";
    if (kind === "skill")
        return "반복되는 처리 방식이 보이면 작은 절차로 고정할 수 있습니다.";
    return input.trim() ? "지금은 자동화보다 한 번 더 손으로 확인하는 흐름이 더 안전합니다." : "요청이 아직 구체화되지 않았습니다.";
}
function capabilityFirstSafeStep(kind, readiness) {
    if (kind === "cron" && readiness === "ready-to-configure") {
        return "먼저 시간, 반복 주기, 할 일, 결과를 받을 위치, 실패했을 때 알림 방식을 확인합니다.";
    }
    if (kind === "cron")
        return "한 번 수동으로 실행해 보고 반복해도 되는지 확인합니다.";
    if (kind === "memory")
        return "기억 후보로 남긴 뒤, 오래 쓸 기준인지 사용자에게 확인합니다.";
    if (kind === "plugin")
        return "기본 OpenClaw 설정을 바꾸지 않고 enable/disable 가능한 방식인지 확인합니다.";
    if (kind === "agent")
        return "작업 목표와 종료 조건을 한 문장으로 고정합니다.";
    if (kind === "skill")
        return "잘해야 하는 일을 3~5단계 절차로 먼저 적습니다.";
    return "한 번의 수동 절차로 해보고 반복되는 부분만 따로 떼어냅니다.";
}
export function translateCapabilityNeed(input) {
...
export function renderCapabilityTranslationReply(input) {
...
function delegationCandidateLabel(candidate) {
    switch (candidate) {
        case "direct_answer":
            return "바로 답변";
...
function delegationLevelLabel(level) {
    switch (level) {
        case "L0_answer_only":
            return "답변만";
        case "L1_draft_only":
...
function delegationFirstStep(report) {
...
export function renderDelegationSurfaceReply(plan) {
...
export function shouldRenderDelegationSurface(plan) {
    const input = plan.currentTurn.cleanInput;
    const asksForDelegationJudgment = /(맡겨도|맡길|위임|delegate|후보|판단|검토|만들까|만들지|스킬로\s*만|에이전트에게|agent에게|자동화할까|자동화\s*후보|cron\s*후보|크론\s*후보)/i.test(input);
    if (asksForDelegationJudgment && /(스킬|skill|에이전트|agent|워크플로|workflow|자동화|cron|크론|맡|위임|delegate)/i.test(input))
        return true;
...
function stateHygieneClassLabel(item) {
    switch (item) {
        case "memory":
            return "기억/메모리";
        case "agreement":
...
function stateHygieneFirstCheck(item) {
    switch (item) {
        case "task_history":
...
export function renderStateHygieneSurfaceReply(plan) {
...
export function shouldRenderStateHygieneSurface(plan) {
    const input = plan.currentTurn.cleanInput;
    if (plan.operatingJudgment.stateHygiene.classes.length === 0)
        return false;
    if (isActiveRuntimeOutageRequest(input))
        return false;
    if (isSurfaceLoopComplaint(input))
        return false;
    const explicitStateHygieneIntent = /(현재\s*문제.*과거\s*흔적|과거\s*흔적.*현재\s*문제|historical\s*residue|residue|메타데이터\s*흔적|남아\s*있는.*흔적|대화창.*시스템|내부\s*문구|system\s*(message|request)|task\s*issue|task\s*pressure|승인\s*요청.*반복|approval.*반복)/i.test(input);
    const explicitSeparationIntent = /(분리|구분|정리|위생|hygiene|오염)/i.test(input);
    const stateHygieneObject = /(task|queued|running|approval|승인|transcript|대화창|system|시스템|memory|메모리|agreement|합의|세션|session|흔적|residue|오염)/i.test(input);
    return explicitStateHygieneIntent || (explicitSeparationIntent && stateHygieneObject);
}
function isActiveRuntimeOutageRequest(input) {
    const activeRuntimeObject = /(텔레그램|telegram|polling|poilling|폴링|gateway|게이트웨이|channel|채널|message|메시지|응답|reply|전송|send|수신|inbound|outbound|연동|접속)/i.test(input);
    const activeOutageSignal = /(끊|끊긴|끊킨|멈|먹통|무응답|응답\s*없|응답을\s*안|안\s*와|안\s*옴|안\s*된다|안\s*돼|failed|failure|error|timeout|timed\s*out|stall|disconnected|not\s*working|조치|해결|복구|진단|확인|문제.*뭔|뭐가.*문제)/i.test(input);
    return activeRuntimeObject && activeOutageSignal;
}
function isSurfaceLoopComplaint(input) {
    const normalized = String(input || "").replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
    const surfaceTerm = /(보류|승인|approval|승인\s*경계|상태\s*위생|할\s*수\s*없|cannot|권한|카드|surface|표면\s*응답|문구|답변)/i.test(normalized);
    const loopSignal = /(계속|반복|똑같|같은|되풀이|무한|고정|stuck|loop|루프|입력하든|말하든|뭘\s*입력|무엇을\s*입력)/i.test(normalized);
    const malfunctionContext = /(멈|먹통|전달.{0,12}안|응답.{0,12}안|대시보드|dashboard|텔레그램|telegram|세션|새\s*세션|두\s*번째|2번째|second|첫.{0,8}정상|설치\s*후)/i.test(normalized);
...
function approvalRiskSummary(report) {
    if (report.risk.strongApprovalRequired)
        return "강한 승인 경계입니다.";
    if (report.risk.approvalRequired)
        return "승인 경계가 있는 작업입니다.";
    return "현재 문장만 보면 강한 승인 경계는 뚜렷하지 않습니다.";
}
function approvalDefaultAction(report, input) {
...
export function renderApprovalBoundarySurfaceReply(plan) {
    const report = plan.operatingJudgment;
...
export function shouldRenderApprovalBoundarySurface(plan) {
    const input = plan.currentTurn.cleanInput;
    if (isSurfaceLoopComplaint(input))
        return false;
    if (shouldRenderRecoverySummary(input))
        return false;
    if (plan.operatingJudgment.approvalErgonomics.riskTransition === "same_scope_continuation" &&
        !plan.operatingJudgment.risk.approvalRequired) {
        return false;
    }
    const asksApprovalJudgment = /(승인.*(해도|눌러도|허용|괜찮|문제|반복|계속)|allow[-\s]?(once|always)|approve|deny|approval|plugin approval|codex_command_approval|명령\s*승인|승인\s*요청)/i.test(input);
    const highRiskExecutionNeedsApprovalSurface = plan.operatingJudgment.approvalErgonomics.shouldAskUserNow &&
        hasHighRiskExecutionSurfaceRequest(input);
    if (highRiskExecutionNeedsApprovalSurface)
        return true;
    if (!asksApprovalJudgment)
        return false;
    if (/(그냥\s*승인해|대신\s*승인|자동\s*승인|우회)/i.test(input))
        return true;
...
export function shouldTranslateCapability(input) {
    return /(자동화|반복|스킬|skill|플러그인|plugin|에이전트|agent|cron|크론|메모리|memory|OpenClaw.*할 수|오픈클로.*할 수)/i.test(input);
}
function isInstallOrUpgradeRequest(input) {
    const normalized = String(input || "").replace(/\s+/g, " ").trim();
    return /(zip|압축|첨부|파일|패키지|package).{0,40}(설치|install|깔|적용|업데이트|교체|업그레이드|upgrade)|(설치|install|깔|적용|업데이트|교체|업그레이드|upgrade).{0,40}(zip|압축|첨부|파일|패키지|package)/i.test(normalized);
}
function isVagueActionFollowup(input) {
    const normalized = String(input || "")
        .replace(/\s+/g, " ")
        .trim();
    if (!normalized || normalized.length > 36)
        return false;
    return /^(야|뭐야|왜|그래서|그래서\s*(조치|해결|고쳐|복구|진단|확인).{0,8}|조치\s*해|조치를\s*해|해결해|고쳐|고쳐줘|복구해|지금\s*문제가\s*뭔데|문제가\s*뭔데|뭔데|어쩌라고)$/i.test(normalized);
}
function isConceptualDiscussion(input) {
    const normalized = String(input || "").replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
    const conceptual = /(개념|철학|방향|설계|기획|계획|의견|생각|비교|차이|원리|구조|상품|성능|토큰|런타임|플러그인|스킬|에이전트|크론|자동화|대화\s*능력)/i.test(normalized);
    const activeFailure = /(안\s*돼|무응답|응답\s*(없|안)|끊|끊긴|끊킨|멈|먹통|failed|error|timeout|stall|disconnected)/i.test(normalized);
    return conceptual && !activeFailure;
}
function stripSurfaceContractMetadata(input) {
    return String(input || "")
        .replace(/\n+Detected install package candidate:\n[\s\S]*$/i, "")
        .trim();
}
function surfaceContractGuidance(kind) {
    switch (kind) {
        case "state_hygiene":
...
export function decideSurfaceIntervention(plan, kind) {
...
function detectRecoveryFamily(input) {
    if (/텔레그램|telegram|polling|poilling|폴링|dm|bot|메시지|메세지|응답\s*(안|없)|무응답|안\s*와|안\s*옴/i.test(input))
        return "telegram_surface";
    if (/gateway|게이트웨이|daemon|launchagent|18789|reachable|event loop|재시작/i.test(input))
        return "gateway_runtime";
    if (/oauth|로그인|auth|인증|model|모델|gpt|openai|토큰/i.test(input))
        return "model_auth";
    if (/plugin|플러그인|hook|hooks|extension|enabled|doctor/i.test(input))
        return "plugin_runtime";
    if (/memory|메모리|기억|context|맥락|오염|agreement|합의/i.test(input))
        return "memory_context";
    if (/session|세션|queue|queued|handoff|이어|분기/i.test(input))
        return "session_queue";
    if (/cron|크론|자동화|반복|schedule|예약|heartbeat|task/i.test(input))
        return "cron_automation";
    return "unknown";
}
function confirmedRecoveryState(input, family) {
    const confirmed = [];
    if (/OK|reachable|running|ready|loaded|통과|정상|응답/i.test(input))
        confirmed.push("일부 상태 신호는 정상으로 보입니다.");
    if (/failed|error|warn|blocked|missing|unavailable|무응답|안\s*돼|실패/i.test(input))
        confirmed.push("문제 신호가 보고되었습니다.");
    if (family !== "unknown")
        confirmed.push(`문제 범위는 ${family} 쪽으로 먼저 좁혀 볼 수 있습니다.`);
    return confirmed.length > 0 ? confirmed : ["아직 확인된 상태 신호가 충분하지 않습니다."];
}
function likelyRecoveryCauses(family) {
    switch (family) {
...
function recoveryUnknowns(family) {
    switch (family) {
        case "telegram_surface":
...
function recoveryNextCheck(family) {
    switch (family) {
        case "telegram_surface":
...
function recoveryFamilyLabel(family) {
    switch (family) {
        case "telegram_surface":
            return "텔레그램 연결";
        case "gateway_runtime":
...
export function mapRecoverySummary(input) {
...
export function renderRecoverySummaryReply(input) {
...
export function recoveryEscalationFingerprint(input) {
    const family = detectRecoveryFamily(input);
    const normalized = String(input || "")
        .toLowerCase()
        .replace(/[`"'“”‘’]/g, "")
        ...
        .replace(/\s+/g, " ")
        .trim();
    const intent = /상태|status|doctor|hooks|tasks|logs|확인|점검|검증/.test(normalized)
        ? "inspection"
...
export function renderRecoveryEscalationReply(input, occurrence) {
    const summary = mapRecoverySummary(input);
    const stage = occurrence <= 1 ? 1 : occurrence === 2 ? 2 : 3;
    if (stage === 1)
        return renderRecoverySummaryReply(input);
    if (stage === 2) {
        return [
...
export function shouldRenderRecoverySummary(input) {
    const normalized = String(input || "").replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
    ...
    const asksForActualInspection = /(상태\s*파악|전체\s*현재\s*상태|현재\s*상태.*파악|충돌\s*지점|접속\s*문제.*찾|문제.*찾|개선안|점검|확인|조사|로그|status|doctor|hooks|tasks|logs|inspect|diagnostic|diagnostics)/i.test(normalized) &&
        /(오픈클로|openclaw|비아이|beai|텔레그램|telegram|게이트웨이|gateway|플러그인|plugin|훅|hook|설치|install|연동|접속)/i.test(normalized);
    if (asksForActualInspection)
        return false;
    const hasOutageRecoverySignal = /(안\s*돼|무응답|응답\s*(없|안)|응답을\s*안|안\s*와|안\s*옴|끊|끊긴|끊킨|멈|먹통|timeout|timed\s*out|stall|disconnected|blocked|unavailable|not\s*working)/i.test(normalized);
    ...
    const hasOperationalTarget = /(오픈클로|openclaw|비아이|beai|텔레그램|telegram|게이트웨이|gateway|플러그인|plugin|훅|hook|설치|install|연동|접속|메모리|memory|task|approval|승인)/i.test(normalized);
    const hasOperationalFailureContext = hasOperationalTarget ||
        /(command|tool|hook|plugin|gateway|telegram|oauth|auth|session|queue|cron|message|send|reply|delivery|로그|상태|실행|전송|메시지|명령|도구|훅|세션|큐|인증)/i.test(normalized);
    const looksLikeConceptualFailureDiscussion = /(가능성|리스크|위험|handling|핸들링|어떻게\s*봐|설계|전략|분석|케이스|테스트|예시)/i.test(normalized) && !hasOperationalTarget;
...
export function updateBeaiMemoryAssets(workspaceDir, plan, finalText) {
    if (!workspaceDir)
        return;
    ...
    const existingMemory = readJsonArray(memoryPath);
    const existingAgreements = readJsonArray(agreementPath);
    const existingDiscarded = readJsonArray(discardedPath);
    const existingAutomationRegistry = readJsonObject(automationRegistryPath);
    const memoryCandidates = mergeUniqueByText(existingMemory, buildBeaiMemoryCandidates(plan, finalText));
    const workflowStateLedger = {
...
export function updateSessionContinuityState(workspaceDir, plan) {
    if (!workspaceDir)
        return;
...
function renderAgreementCandidates(plan, finalText) {
    ...
    lines.push("", "## Promotion Rule", "", "- promote only after explicit review or approval", "- do not copy tool output or transient task chatter into accepted agreements", "- prefer project state when the item is still project-local rather than cross-cutting");
    return `${lines.join("\n")}\n`;
}
export function updateAgreementCandidates(workspaceDir, plan, finalText) {
    if (!workspaceDir)
        return;
