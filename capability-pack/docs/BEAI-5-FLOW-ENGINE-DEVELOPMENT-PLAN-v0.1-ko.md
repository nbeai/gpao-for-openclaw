...
Status: phase-8 human companion quality contract implemented
Date: 2026-07-02
...
사용자가 BEAI 5를 "똑똑하다"고 느끼는 이유는 더 많은 정보를 말해서가 아니라, 사용자의 현실과 판단 흐름을 같이 붙잡기 때문이다.

따라서 BEAI 5의 성능을 패키지에 녹인다는 것은 다음을 뜻한다.

- 따뜻한 말투를 복사하지 않는다.
- 공감 템플릿을 강제하지 않는다.
- 현재 요청을 첫 기준으로 둔다.
- 이전 맥락은 현재 요청을 돕는 보조 맥락으로 둔다.
- 사용자의 인지 부담을 줄인다.
- AI가 사용자의 선택권을 몰래 가져가지 않는다.
- 대화 끝에는 판단 기준, 다음 행동, 복구 경로, 또는 바로 쓸 산출물이 남는다.

이 기준은 `BEAI Human Companion Quality Contract`로 패키지에 추가되었다.
...
2026-07-02부터 Flow State Spine 위에는 다음 품질 프로필이 추가된다.

```text
HumanCompanionQualityProfile
```

이 프로필은 별도 엔진이 아니라 기존 런타임이 매 턴 계산하는 대화 품질 게이트다.
...
목표는 사용자가 느끼는 "비아이5식 동반감"을 말투가 아니라 검증 가능한 상태로 만드는 것이다.
...
### Phase 8. Human Companion Quality Contract

목표:

- BEAI 5의 인간 동반형 대화 성능을 패키지의 공식 런타임 계약으로 승격한다.

작업:

1. `config/beai-human-companion-quality-contract.json`을 추가한다.
2. `docs/BEAI-HUMAN-COMPANION-QUALITY-CONTRACT-v0.1-ko.md`를 추가한다.
3. `runtime-core.ts`에 `HumanCompanionQualityProfile`과 `buildHumanCompanionQualityProfile`을 추가한다.
4. `renderPromptContext`에 `human_companion_quality` 섹션을 추가한다.
5. `beai-flow-regression-gate.mjs`에 human companion quality 회귀 항목을 추가한다.
6. `beai-user-scenario-audit.mjs`에 S19 human companion quality 시나리오를 추가한다.
7. `beai-doctor-package-check.mjs`가 계약 파일과 문서를 required file로 확인하게 한다.

완료 기준:

- BEAI 5의 성능을 "따뜻한 말투"로 오해하지 않게 문서화한다.
- 현재 요청 우선, 인지 부담 감소, 사용자 선택권 보존, 장기 맥락 안정, 신뢰 회복, 바로 쓸 다음 움직임이 런타임 상태로 계산된다.
- package verify가 관련 게이트를 실행한다.
