...
## Telegram direct 진행 상태 계약

긴 작업은 사용자가 멈춤으로 느끼기 전에 진행 상태를 보여야 한다.

- 첫 상태 보고: 30초 이내
- 장기 작업 중 진행 업데이트: 최대 120초 이내
- 진행 업데이트 내용: 진행 중 / 검증 중 / 막힌 지점 / 다음 행동 중 현재 상태에 맞는 항목
- hook registration 의심 신호: 조용히 숨기지 않고 BEAI Doctor repair plan으로 올림

자동 주기 발송, Gateway 재시작, hook 등록, cron/agent 등록은 별도 승인 경계다. 이 계약은 "보여야 하는 기준"을 고정하며, 승인 없는 외부 전송 자동화를 만들지 않는다.
